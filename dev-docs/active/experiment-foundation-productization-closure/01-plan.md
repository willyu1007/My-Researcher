...
8. **Remaining named-local authority gate:** after explicit approval, apply exactly 2 DataPolicy identities/revisions, 2 Dataset identities/revisions, 4 freeze receipts and 10 lifecycle events through the existing service/repository boundary; require exact replay and protected-table negative-space verification. Then bind both mirror entries to the returned exact Dataset refs.
