...
6. **Remaining pre-live gate:** decide the provider-native immutable content identity accepted for the official PAI image. The known versioned `ImageUri` and stable PAI `ImageId` remain provider asset evidence, not an OCI/content digest.
7. Direct commits to `main`, without a PR, are the owner-confirmed delivery strategy; do not rewrite or force-update history.
8. Immediately before the first `CreateJob`, require a fresh short-lived controller STS supplied outside the repository and the exact in-session authorization string with date, ¥50 ceiling and two-job limit. Keep both capabilities false until then.
9. Run M7-L1 as diagnostic-only and require `real_provider_canary_passed`, terminal/cleanup verification, zero duplicate jobs and zero ExperimentResult/EvidenceCandidate/REU. M7-L2 remains a separately authorized scientific closure.
10. Before marking T-132 `done`, resolve the four non-M7 audit remainders: EF-P06 product candidate promotion, EF-P14 bound bootstrap/legacy-null handling, EF-P15 standalone attachment/revalidation, and EF-P21 semantic retrieval (already deferred by D-24). Each needs either implementation evidence or a named follow-up task; no silent scope loss.
