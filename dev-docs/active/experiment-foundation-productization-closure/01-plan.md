...
10. **Blocked by final zero-write preflight 2026-07-28 — executable lineage apply:** named-local has exactly one WorkOrder revision and one Run, both v1 simulation lineage; the RunRecipe has null `executionBundleRevisionId/Hash`. D-23 forbids dispatching or trust-upgrading that immutable Run. Before any cloud submit, separately authorize and apply one successor WorkOrder revision v2 with two exact cells and the frozen ExecutionBundle ref, then drain normal T1-T4 to one new executable Run/head/acknowledgement. Expected maximum is 40 new rows plus the existing branch CAS updates; replay must create zero rows and scientific/evidence/provider-control families must remain unchanged.
