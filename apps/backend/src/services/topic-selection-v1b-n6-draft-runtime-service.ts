...
export function buildV1bN6DraftSystemContent(hasDecisionMemory: boolean): string {
  const base = [
    ...
    'For each candidate keep answerability_plan.datasets_or_resources, metrics, baselines, and evaluation_setting non-empty, keep observable_success_criteria non-empty, give at least one non-weak falsification_condition, hold expected_claim, fallback_claim, and max_claim_strength within the selected ResearchSlice claim ceiling, and set source_validated_need_refs to the frozen slice validated need ref.',
...
export class TopicSelectionV1bN6DraftRuntimeService {
  ...
  private messages(
    ...
    return [
      {
        role: 'system',
        content: buildV1bN6DraftSystemContent(Boolean(contextPacket.decision_memory)),
      },
      {
        role: 'user',
