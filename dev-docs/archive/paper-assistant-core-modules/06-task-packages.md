...
## Owner and handoff mapping (confirmed)
| Package | SSOT owner | Primary implementation lane | Handoff dependency |
| --- | --- | --- | --- |
| TP-01 文献管理 | `T-002` (package definition) | backend-first + desktop panel | Batch A start gate |
| TP-02 研究方向池 | `T-002` (package definition) | backend-first + desktop panel | TP-01 |
| TP-08 论文管理 | `T-002` (package definition) | backend-first | TP-01 |
| TP-03 理论框架与研究设计 | `T-002` (package definition) | hybrid | TP-02 |
| TP-04 实验设计 | `T-002` (package definition) | backend-first | TP-03 |
| TP-05 模型与训练 | `T-002` (package definition) | backend-first | TP-04 |
| TP-06 数据分析与讨论 | `T-002` (package definition) | hybrid | TP-04 + TP-05 |
| TP-07 写作、投稿、修稿 | `T-002` (package definition) | frontend orchestration + backend checks | TP-01~TP-06 |

## Execution-ready queue (next tasks)
1. Batch A queue: TP-01 -> TP-02 -> TP-08
2. Batch B queue: TP-03 -> TP-04 -> TP-05 -> TP-06
3. Batch C queue: TP-07

Notes:
- `T-002` 仅维护任务包定义与批次顺序，不维护阶段门禁与版本字段。
- 阶段门禁、版本主线、快照冻结规则统一由 `T-003`（已归档）产物继承。
