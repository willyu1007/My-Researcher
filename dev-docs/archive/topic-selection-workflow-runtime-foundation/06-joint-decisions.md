...
## D-30 (2026-07-07) — v1b N8 operator 强制辩论门（T-OP）+ N6/N8 debate 阈值重定性 advisory（联合决策）
- **背景**：产品化审查（2026-07-07）核实 N8 debate 全链现状：T1/T3 确定性触发 → `n8_feedback_to_n7` loopback → N7 `n7_n8_debate_admission_review` 选 debate_level → coordinator frontier 辩论 **opt-in**（caller 可给 `node_inputs.debate` 跑辩论、或给 `draft_payload` 单 agent 直过，重入触发码降级 warning，DP-3.2）。即设计已是"触发=提醒+开门，人做最终决定"。剩余两个实质缺口：(a) **阈值是进辩论的唯一门**——`pendingN8BoundedDebate` 只认 loopback 存在，矩阵 Node Policy Notes 设想的"operator 强制"从未实装，阈值假阴性=辩论不可达；(b) 阈值被定性为"待 C-1 标定的质量门"，背着 W-06/W-07 provisional product tripwire + W-15 `provisional_threshold_run_override` 签核（每次产品 N8 都发警告并停等签核），而 C-1（≥100 多 provider 样本，FP<5%）外部语料 gated 无时间表——产品长期"半吊子"。
- **决定（用户拍板 2026-07-07，两项）**：
  1. **T-OP operator 强制触发源（N8）**：run request 新增可选 `operator_debate_request { reason, requested_by }`（N8-only，其余节点 fail-closed 400；提交时声明——**不支持事后（admit 后）重入反悔**，未来若需事后路径必须另开 JD 且带人工批准门）。首过时与 T1/T3 同列合并进同一 loopback 触发（新码 `N8_OPERATOR_FORCED_DEBATE_TRIGGER`），走**完全相同**的既有链（N7 准入选 level → frontier opt-in → 反振荡 firstPass 守卫对 T-OP 同样生效，operator 亦不能制造循环；重入重复声明降级 `N8_OPERATOR_FORCED_AFTER_DEBATE` warning）。零新路由、零新 gate（DMP-10）；coordinator `node_inputs` + HTTP advance 路由 schema 同步透传。
  2. **阈值重定性 advisory routing heuristic**：N6/N8 debate 阈值从"待标定质量门"改判为"advisory 路由启发器"——依据：它们只开一扇人最终决定的门（N8 frontier opt-in；N6 仅 gate 失败可达且 escalation 判定本就在 caller triage 工件），且 T-OP 落地后辩论可达性与阈值质量彻底解耦（假阴性→人随时可显式开门）。据此 **supersede** W-06/W-07 provisional product gate + 退役 W-15 advance 侧 `sign_off_required` 强制（harness 不再发射 `N6/N8_DEBATE_THRESHOLDS_PROVISIONAL` 警告；coordinator 不再停等签核）。**C-1（原 W-13/W-17 标定）从发布 blocker 缩为可选启发器调优**；T-129 范围相应收窄为 C-2（prompt 正文）+ C-3（provider debate 翻门）。与 D-27 同构：消解风险的方式从"门禁+签核"变为"该值不 govern 任何产品结果"。
- **边界/不改动**：`provisional: true` 字段保留（仅记录"未经验标定"事实）；**翻 flag 仍是人工代码变更**（T-127 D8 record-and-defer 对 flip 本身不变），标定工具链（runner/analysis/materializer + dry-run 脚本）原样保留；签核**记录**路径（route + 严格校验 + tripwire attempt 锚定 + 幂等）保留合法（服务历史带警告的 run），仅 advance 侧强制退役；W-14 provider debate dormancy（C-3）不触碰；T1/T3 计算函数、loopback 持久化形态、replay 哈希组件全部不变。
- **验证**：shared schema tests 30/30（新增 operator 字段 schema 正负例 + D-30 supersede 断言）；harness 单测 101/101（新增：T-OP 干净首过 armed loopback、非 N8 400、空字段 400；N6 escalation 产品跑不再带 provisional 警告）；coordinator 单测 60/60（4 个 W-15 D1(c) 用例改判 D-30 语义：警告 run 不再 halt，记录路径锚定保持）；v1b routes 集成 12/12（Prisma smoke 需 DATABASE_URL 为环境项）；标定 4 文件 27/27；矩阵一致性 + 27 自测负例全绿。`topic-selection-w15-s4-signoff-product-run.mjs` 按 D-28 判据退役删除（其断言行为不复存在），08-scenarios 登记行注记留痕。
- **归属**：T-088 域（harness/coordinator touch，按 D-T128-00 登记）；产品裁决用户拍板 2026-07-07。冲突面：contracts + harness + coordinator + v1b 路由 schema + 桌面 RunOperationsCard 文案 + calibration dry-run 头注释。
