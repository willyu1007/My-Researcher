...
2. **Completed 2026-07-28 — B / RAM:** both custom policies and separate roles are created and console-verified. Controller policy v2 is active at repository SHA-256 `c014cac58a794f2bc4849c0c05993ee85fc660dcb6d3206438b08bf7d5c219be`; v1 is retained as rollback. Runtime policy remains v2 at repository SHA-256 `1eb7de00aceacc14817b058291eb4f2e85cdbb4c10ea467c91084a75094b1a4b`, with `ListObjects` restricted to `input` / `input/*` and object read/write statements separated. Controller role `300042892692129613` trusts only owner user `acs:ram::1183869713036194:user/user_0002`; runtime role `300525928077898732` trusts only PAI service `pai.aliyuncs.com`. Each role has exactly its matching custom policy and no cross-attachment.
...
14. Obtain a fresh ≥55-minute controller STS and a new action-time paid-window confirmation. Run M7-L1 as diagnostic-only and require `real_provider_canary_passed`, terminal/cleanup verification, zero duplicate jobs and zero ExperimentResult/EvidenceCandidate/REU. M7-L2 remains a separately authorized scientific closure and requires OCI/content-digest image identity.
15. Before marking T-132 `done`, resolve the four non-M7 audit remainders: EF-P06 product candidate promotion, EF-P14 bound bootstrap/legacy-null handling, EF-P15 standalone attachment/revalidation, and EF-P21 semantic retrieval (already deferred by D-24). Each needs either implementation evidence or a named follow-up task; no silent scope loss.
...
10. [ ] Obtain a fresh ≥55-minute controller STS and new action-time paid-window confirmation, then execute/collect the exact two-job diagnostic under the ¥50 ceiling; run exact replay and protected/scientific negative-space verification.
