...
8. **Remaining named-local authority gate:** the restart-safe importer, exact replay, drift rejection, protected-table negative-space checks and fail-closed authorization gate are prepared offline. The first authorization request counted 22 rows but omitted the four lifecycle projections that the repository must maintain with the 10 append-only events; the incomplete request therefore authorized no apply and no database write occurred. After supplemental approval, apply exactly 2 DataPolicy identities, 2 DataPolicy revisions, 2 DataPolicy freeze receipts, 2 Dataset identities, 2 Dataset revisions, 2 Dataset freeze receipts, 10 lifecycle events and 4 lifecycle projections (26 rows total) through the existing service/repository boundary. Require exact replay with zero new rows and protected-table negative-space verification, then bind both mirror entries to the returned exact Dataset refs.
9. **Remaining bundle gate:** create/freeze the reviewed v2 bundle from those exact bindings, rerun the default-off same-payload gate, and perform a fresh read-only `GetImage` comparison immediately before live submission.
