...
| Current N2 frontier | Next node `topic-selection.v1b.record-research-constraint-profile.v1`; zero attempts. Proposed profile preserves `k=0/1/5`, signed `0→1`/`1→5`, fixed non-depth pipeline, direct confidence/binary-utility/query-complexity baselines, separate benefit/harm/call/token/trigger/latency accounting, one-workstation scope, and no provider or N8 execution. It is advisory until the researcher exactly accepts it. |
| Current-lineage boundary proof | A is closed / `human_confirmed`, C remains `hypothesis` / `machine_checked` as the viable alternative, the current v1b bundle and N1 intake snapshot exist, and N2 remains unwritten. No new constraint profile, ResearchSlice, question/package/v1c owner, HumanPromotionDecision, PromotionDecision, bridge, PaperProject, provider call, or corpus mutation occurred. |
| Current v1c frontier | Machine Gate remains independently `ready_for_human_decision`, but the old question/package/v1c/promotion chain is stale relative to the repaired gap lineage and must not be retried. The earliest unsatisfied point is strict-human N2 on the current run; after N2, run N3/N4, stop at N5, then rebuild N6/N7 and present the newly snapshot-bound question confirmation. |

## Resume guard

The historical evidence decisions remain durable, both earlier candidate arenas remain readable, and their human-confirmed ValidatedNeed is preserved as provenance, but the old gap authority is upstream-stale relative to refreshed evidence checkpoint `research_checkpoint_19a6227d-3849-4d0e-ab6f-9e89e072b457`. Preserve the negative arena as a non-advancing control. The bounded two-paper acquisition, parsing, curation, and EvidenceMap refresh are complete, and downloader `max_byte_size` is restored to `1024`. Current-evidence gap checkpoint `research_checkpoint_de57273c-4b2e-4820-9f98-898920a6fe63` is decided `advance`; A is the human-confirmed source of current ValidatedNeed `validated_need_a58766a3-d482-4dc3-9b0b-cdeac9e3d095`, while C remains the unselected viable alternative. Current v1b bundle `v1b_input_bundle_fc913271-74bb-4359-b19b-0d4216a609bc` and run `workflow_run_t147_a9_pos_v1b_current_gap_v1` are durable; N1 is admitted and N2 has zero attempts. Resume only by presenting the exact N2 profile against intake snapshot `v1b_intake_snapshot_8641862d-753a-4cf3-8ac9-4d5f90ec89f3` and recording it after explicit researcher acceptance. Do not retry old promotion checkpoint `research_checkpoint_35406533-fac3-4cdb-b6d9-29296ca66c63` or its stale downstream lineage. Stop again at N5 and at the new question checkpoint; do not invoke a provider, advance N8, or create a bridge without the corresponding new authorization boundary. The configured raw-file root is transient; if its OA PDFs disappear, obtain exact authorization and reacquire only those two assets through a new bounded dry-run rather than overwriting storage settings.
