...
## Current execution state — 2026-07-23
...
- The roadmap remains `in-progress`: M0-M4 technical/product-bound control work and the M6 read-only cloud boundary are verified. Real provider execution/scientific activation, UI/search and release/non-local rollout remain incomplete.
- M7 readiness freezes a new-lineage design: the existing immutable simulation Run cannot be dispatched as real; a new PI WorkOrder revision binds an exact typed ExecutionBundle and creates a new Run through T1-T4. T-132 owns M7; T-106 only consumes its evidence. Next gate is independent default-off M7 code/schema/test implementation authorization; any `CreateJob`, migration apply, scientific enable or non-local rollout remains separately fenced.
...
- [ ] Obtain independent authorization for M7-I0 through M7-I3 default-off code/schema/migration-file/test implementation; that authorization does not cover migration apply or any provider/OSS write.
