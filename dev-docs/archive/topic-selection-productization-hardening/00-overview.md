...
## Status
- State: done
- Closure / 移交（2026-06-16 收尾关闭归档）: Phase 0–4 + 5.2/5.3 全部 DONE；**两条开口线所有权移交 T-127**（`topic-selection-backend-hardening-and-expansion`）——**F-11 harness 单文件拆分 → T-127 W-12**（承 D-T123-03，一次拆透）、**DP-3.3 N8 阈值标定 → T-127 W-13**（record-and-defer，语料门控）；F-10/scaffold 未落地工作树残留 → T-127 W-01 落地。本包不再持有进行中工作，避免与 T-127 双轨。详见 `03-implementation-notes.md` §2026-06-16 收尾移交、T-088 `06-joint-decisions.md` D-T123-03 续推指针、T-127 `00-overview.md` D7。
...
- Progress: **Phase 5.2（F-10）DONE（2026-06-15）**——F-10 关闭。per-provider token 估计落地：estimator `provider_id`-keyed 校准表（**真实分词器实测** o200k_base / Qwen-BBPE / DeepSeek-V3，CJK tok/char 取保守 0.90/0.95/0.85 > 实测均值 0.84/0.73/0.67（取值由整输入最差边际封顶，非裸 CJK 比——dashscope 裸 CJK 最省却整输入边际最薄，故最少收紧），**default/unknown byte-identical**）+ budget gate 透传 `provider_id`。一轮对抗性多角度复核（4 lens + skeptic，14 agent）：修 1 真回归（newline-dense CJK 低估 → newline 项 provider 收紧 ÷1）、核证 2 误报安全（paper-impl identity hash / compression-runtime 不一致）、余既有/范围外限制已记。1200 输入（33% newline-heavy + 25% CJK-heavy）×1.25 **0 低估**（最差 ≈1.04）；套件 1314/0/35skip、tsc 0、debate byte-identity 保持。压缩策略去硬编码同期收。STEP-7 debate 压缩 facts 仍独立 consumer-dependent 延期（非 F-10 阻断）。详见 03 §2026-06-15 F-10 per-provider token 校准。
- Next: **已收尾关闭**。原"Phase 5 仅剩 5.1（F-11 harness 拆分，slice 1 已交付）"的余项与 DP-3.3 标定余项**移交 T-127 W-12 / W-13**（见上 Closure）；本包状态终态 done。
- Task ID: `T-123`
