...
## Current execution state — 2026-07-15
...
- Named local-development landing and deep cleanup are complete: migration `20260714210000_normalize_experiment_v2_event_payloads` (`37eed54494aa2dc246fe7d5a9b2de2a027474b416ada8a5f18ab4bc194a65f3a`) is applied only there, migration history is 62/62 and Pack B 6/6 tables remain empty. The migration normalizes event storage, hardens 38 FKs and fences nine fixed-v1 columns; PostgreSQL `Int` input boundaries are enforced before persistence. Pre/post Pack A semantic authority is 208 rows at `sha256:494cdf5a02e2379a66a12bc82411e8237f39e949a2f992f3e12a0e220f613d74`; legacy is unchanged at 257 rows / `sha256:f9e7a6875c9a3597ecb485abc978920e461a3af33f071c676665439dde17211d`.
...
- The roadmap remains `in-progress`: M0-M2's bounded Pack A control-plane source-binding slice and M3/Pack B technical slice are verified; scientific/closure/UI/release phases remain incomplete.
- Next gates are ordered: establish a formal product-bound PaperProject/active ValidationCycle and complete Pack A admission-to-ack before separately authorizing named-local Pack B E1-E5; then authorize real read-only preflight, scientific/D-18 closure and UI/search slices independently.
- Source defaults and current named-local admission/cutover/simulation overrides are all off. Historical enabled app-composition probes remain guard evidence only. The named local-development database/config changed only under explicit authorization; Pack B changed no non-local database, product traffic, legacy writer or provider.
...
| M7 Real provider execution and evidence closure (deferred) | optional minimum Aliyun execution behind explicit budget/credential/data/cleanup approval | outside the first release door; only this or another verified real provider may close real result, EvidenceCandidate and RunEvidenceUnit production claims |

Current milestone state: M0-M2 are verified for the bounded Pack A control-plane source-binding slice. M3-M7 remain planned/in progress according to their independent authorization and evidence gates.
...
- D-19 boundary:
  ...
  - D-22 excludes candidate/import/promotion and all Attempt/provider/result/validation/evidence/closure/UI/search/legacy-mapping persistence from this migration; exact typed fixture asset kinds are locked by the implementation-readiness matrix;
...
- Planned product gate:
  - Phase 0 freezes one command, output schema, required check IDs and artifact root for invariants, scientific-policy tests, durable provider control, cross-module control trace, UI interaction and action budget; Phase 6 executes that predeclared gate without redefining it.
- Governance/docs:
...
- [ ] Establish formal product PI scope, drain Pack A to the exact EF acknowledgement, then run named-local Pack B E1-E5 without fixture bootstrap.
