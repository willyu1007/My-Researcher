...
| Current v1c frontier | Title-card lists contain one promotion gate, zero promotion decisions, and zero PaperProject bridges. Preparation is complete; stop before strict-human promotion. The package's claim ceiling and nearest-work, sparse-label, subgroup, cost/harm, and baseline risks remain review inputs even though the gate arrays are clear. |

## Resume guard

The historical evidence decisions remain durable, both candidate arenas are materialized, and the selected positive candidate remains a human-confirmed ValidatedNeed. Preserve the negative arena as a non-advancing control. The bounded two-paper acquisition, parsing, curation, and EvidenceMap refresh are complete, and downloader `max_byte_size` is restored to `1024`. Current checkpoint `research_checkpoint_2be0d1f1-f5c4-4627-94a8-ca9fe91897ad` is exactly decided `advance` against snapshot `8ceaa149...760d`; provider-free N8 plus deterministic N9-N11 are admitted, the recovery v1b run is complete, and deterministic v1c preparation is complete through gate `promotion_gate_check_e24c1cd7-5205-4f9c-8b27-5e5100d996f1`. Do not invoke a provider, record a human promotion decision, or create a bridge without a new authorization boundary. Any promotion decision must bind current promotion-input snapshot hash `1cde2ea96d33041ecda8d35aec91feb7d217505129d3e2e1d2a782d3e85f9605` and explicitly review the package's carried risks. The configured raw-file root is transient; if its OA PDFs disappear, obtain exact authorization and reacquire only those two assets through a new bounded dry-run rather than overwriting storage settings.
