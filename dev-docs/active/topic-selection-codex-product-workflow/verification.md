...
| Real model qualification / CX-02, CX-08 | Pinned abstract cases through attempt 52 | N6 insufficiency/apparent conflict and exact-delta positive/negative review inspected; ordinary N8 v4 admitted | 1,260,338 reported tokens plus unknown usage from four failed attempts; conditional N8 and regeneration qualification remain. All profiles closed. See current evidence below. |
...
`/tmp/my-researcher-t153-phase2/live/`. Sources and setup boundaries are defined above. The accounting snapshot through attempt 52 is **1,260,338 reported tokens and four unknown-usage attempts (4, 8, 9, 20)**.
...
time are null under the user's amendment; the original product timeout is 600 seconds. The next regeneration live run is ongoing in the same ledger; no budget approval is
...
reasonable; the overclaim findings remain supported and undowngraded. No material semantic finding
...
- Complete real v4 conditional N8, actual N6 regeneration and final-gate overclaim refusal using
