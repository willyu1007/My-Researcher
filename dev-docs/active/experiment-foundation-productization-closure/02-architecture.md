...
- Read-only `GetImage` verification resolved PAI asset `image-liuxvj7p2qcnflha84` to the exact regional `ImageUri` `dsw-registry-vpc.cn-shanghai.cr.aliyuncs.com/pai/torcheasyrec:1.3.0-pytorch2.12.1-cpu-py311-ubuntu22.04`. The API returned null `Identity` and `Signature` and no OCI/content digest. The address and provider asset identity are therefore resolved, but a live-capable ExecutionBundle remains blocked on an explicit content-identity contract decision. A versioned tag, PAI `ImageId` or metadata hash must not be written into the existing `container_image.image_digest` field as if it were a content digest.
