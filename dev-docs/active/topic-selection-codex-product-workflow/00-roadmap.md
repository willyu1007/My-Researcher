...
- Current implementation: Sampling is enabled with stable submission recovery and real qualification. Original-abstract extraction and single/complete Debate need discovery are enabled after two real source cases and exact receipt replay; Need adjudication and exact Human-confirmation support now pass real qualification and JSON receipt replay; the three-role convergence chain now passes source-grounded CLI qualification, policy-boundary checks and exact replay; optional Arena also passes original-abstract qualification and exact replay; fresh upstream lineage remains.
