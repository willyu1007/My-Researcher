...
## 2026-08-01 — exact controller workspace-read authorization

- Alibaba Cloud support ticket `000F4RKW18` traced the rejected `CreateJob` path to a workspace-name lookup performed before Job creation and reported the missing action `paiworkspace:GetWorkspace`. The repository call boundary signs `CreateJob` with the controller STS; the runtime role is injected only through `CredentialConfig` for an admitted Job.
- The owner explicitly authorized adding only that action to `pea-m7-canary-controller` and publishing the result through the active Alibaba Cloud Chrome session. `workloads/ragperf-canary/ram/controller-policy.json` now contains one exact Allow statement for `acs:paiworkspace:cn-shanghai:1183869713036194:workspace/1450165`; the runtime policy and both role trust policies remain unchanged.
- The updated repository policy is valid JSON and has SHA-256 `6566a47ee9c07ce6a75c9aeedcbc721d299ae52e7620bbbf91e14564b04220d8`.
- Pre-write Cloud Shell inspection found only `v1`, `v2` and default `v3`, so no version deletion or rotation was required. One initial `CreatePolicyVersion` command included unsupported CLI flag `--PolicyType`; CLI 3.4.11 rejected it before the API call with exit code 2, and no policy version was created.
- The corrected official RAM CLI call created `v4` at `2026-08-01T08:07:44Z` and set it as default. RequestId: `019FBC5D-615E-5915-B388-0612E619C520`.
- Independent read-only verification reported `v1/v2/v3=false` and `v4=true`; the downloaded v4 document hash exactly matched the repository hash. A fail-closed `jq` assertion found exactly one `M7L1ControllerGetExactWorkspace` statement with the approved Action/Resource and returned 0.
- Both Cloud Shell temporary policy files were mode `0600`, removed immediately after their respective calls and absent from the repository. No STS was issued, no `GetWorkspace` or `CreateJob` request was made, and provider Job/database/capability/scientific writes and billable runtime remained 0.
- The RAM change closes the known authorization configuration gap only. Functional `GetWorkspace` verification requires a separately authorized fresh controller STS; any paid Job retry still requires a new dated Job-count and monetary ceiling.

## 2026-08-01 — one-shot controller GetWorkspace verification

- The owner explicitly authorized a temporary controller STS, one read-only `GetWorkspace(1450165)` service call and immediate credential cleanup. No Job submission or paid execution was authorized by this step.
- Each controller session used `DurationSeconds=900` and an inline session policy containing only `paiworkspace:GetWorkspace` on `acs:paiworkspace:cn-shanghai:1183869713036194:workspace/1450165`. The AssumeRole result was fail-closed checked for an `STS.` access-key prefix and the `pea-m7-canary-controller` assumed-role ARN before use.
- The first CLI attempt stopped locally because CLI 3.4.11 could not infer the `aiworkspace` Shanghai endpoint. It returned no provider RequestId and therefore did not send a `GetWorkspace` service request. Its temporary STS and response paths were removed before retry.
- A fresh 900-second session then called the official Shanghai endpoint `aiworkspace.cn-shanghai.aliyuncs.com` exactly once. The provider returned `WorkspaceId=1450165`, `Status=ENABLED` and RequestId `019FBC8E-4067-5E2E-9055-FD3C5D674F28`.
- Credentials were injected only into the bounded subshell through Alibaba Cloud environment variables with profile lookup disabled. The credential and response files were mode `0600`, removed by the exit trap, and independently confirmed absent after the command.
- Effect census for the complete verification: AssumeRole credential issuances 2, actual `GetWorkspace` service requests 1, `CreateJob` requests 0, PAI Jobs 0, database/capability/scientific writes 0, billable runtime and incremental cost 0.
- The exact controller permission is now functionally verified. Sequence 8 remains terminal and its two paid-call allowance remains exhausted; any new Job retry still requires a new dated Job-count and monetary ceiling.
- Using the owner's standing authorization to reply, the credential-free result was posted to support ticket `000F4RKW18` at `2026-08-01 17:12:45` Asia/Shanghai. The reply reported the exact controller grant, successful `WorkspaceId=1450165 / Status=ENABLED` read and provider RequestId, stated that temporary credentials were removed and that no new `CreateJob` had been submitted, and asked the engineer to confirm whether the original request shape can now be retried or whether propagation/other prerequisites remain. The ticket moved from `待您反馈` to `处理中`.

## 2026-08-01 — sequence-9 bounded provider acceptance and recovery

- The owner authorized exactly one PAI Job on 2026-08-01 with a total ceiling of ¥25. The runner token is exact and source-bound; `sequence9-probe` has a hard maximum of one `CreateJob` call and writes no database, capability, scientific result or evidence row.
- A fresh 3,600-second controller STS was checked for the exact assumed-role identity, current controller-policy hash and sufficient remaining lifetime. The sole `CreateJob` call created Job `dlc1b1qk22drb4au` (`ef-v2-real-1-86d9e9fec835b388729f31d3`). Status advanced through `Creating`, `EnvPreparing`, `Running` and `Succeeded` without a second submission.
- Provider timestamps were create `2026-08-01T09:23:55Z`, submitted `09:25:24Z`, running `09:26:05Z` and finished/succeeded `09:26:13Z`. The observed `Running` interval was about 8 seconds and the full create-to-finish interval was 138 seconds, safely below the authorized ceiling for the selected CPU workload; final billing remains provider-owned.
- The initial exact sync stopped after creation because `GetJob` echoed omitted optional request fields as empty strings. Structural read-only diagnosis isolated empty `ResourceId`, credential-role `Policy` and empty `ResourceConfig` scalars; no scientific/output claim was made from the failed comparison.
- The transport comparator now normalizes only empty-string provider echoes to absent values at those nullable string boundaries. Non-empty differences remain fail-closed. A focused fake-provider unit test reproduces the echo shape and proves exact recovery.
- `sequence9-recover` rebuilds the exact first cell with `maximumCreateJobCalls=0`, performs discovery/sync/collection only and cannot submit another Job. It returned schema `t132-m7-l1-sequence9-recovery-result@v1`, status `real_provider_probe_recovered_and_collected`, provider status `Succeeded` and result-manifest hash `sha256:40a5efb0cffc55ddd91dd0dfa495761508c1b3b2aeaec3b0dfe5e592665edd86`.
- Effect census: `CreateJob=1`, accepted/succeeded Jobs `1/1`, recovery `CreateJob=0`, database writes 0, scientific/evidence writes 0, capability persistence 0 and secret output 0. Historical sequence-8 Attempts/commands remain unchanged.
- Cloud Shell source, exact local `/tmp` credential file and Downloads residuals were removed and independently verified absent. Sequence 9's one-call authorization is exhausted.
- T-132 remains `in-progress`: the diagnostic Job is not a durable Pack B Attempt/output and covers only the first cell. The next execution must use a fresh immutable successor and a separately authorized normal two-cell database-controlled flow.

## 2026-08-02 — sequence-9 commit handoff

- Commit scope is limited to the exact controller workspace permission, provider empty-string echo normalization, its focused unit coverage, zero-create sequence-9 recovery and the T-132 handoff documentation. Unrelated dirty-worktree changes remain outside the task commit.
- The consumed `sequence9-probe` runner mode and its dated one-Job/¥25 authorization string were removed before landing. No environment value can reactivate that historical write window. The retained `sequence9-recover` mode constructs the transport with `maximumCreateJobCalls=0` and remains read/recovery-only.
- Current task state remains `in-progress`; this checkpoint proves the prior provider blocker is resolved and one exact cell succeeds, but does not create the fresh successor or the two durable Pack B Attempts required for closure.
- Next three actions: (1) materialize and review a new immutable two-cell successor lineage; (2) run zero-cloud/offline database, payload, replay and ceiling gates; (3) request a new dated two-Job/cost authorization only after those gates pass, then run normal durable execution and exact replay.
- Verification commands and results are recorded in `04-verification.md`. The commit uses conventional subject `fix(m7): land sequence 9 provider recovery` and exactly one `Task: T-132` trailer.

## 2026-08-02 — durable two-cell successor source preparation

- Added the immutable database revision sequence-9 scope to `apply-experiment-foundation-m7-executable-lineage.ts`. The exact parent is sequence 8, branch state/head `16/8`; the new revision/Run/business/id scopes use `durable_two_cell_successor_v9`, and the existing successor census still requires exactly 40 T1-T4 rows with protected and prohibited effects unchanged.
- The new WorkOrder title/objective describe the normal database-controlled two-cell PAI closure rather than another provider-shape diagnostic. Exact cells, frozen ExecutionBundle, `2 CPU / 8192 MiB`, one-attempt and 1800-second policy continue to derive from the immutable parent and existing request builder.
- The new named-local authorization value is intentionally `null`. Supplying any environment value selects the reviewed scope but fails before database connection; the source checkpoint cannot materialize T1-T4 without a later dated max-40/no-cloud authorization.
- Split the live runner's target into two exact scopes. `sequence9-recover` remains bound to historical sequence-8 Run/hash/business identity and constructs a zero-create transport. Normal `offline-preflight`, `image-preflight` and `execute` target the new sequence-9 Run/business identity.
...
## 2026-08-02 — durable successor named-local materialization closure

- The authorized apply completed normal T1-T4 for revision `pi_experiment_revision_v2_t132_m7_l1_durable_two_cell_successor_v9_1` and Run `ef_run_v2_t132_m7_l1_durable_two_cell_successor_v9_1`. Exact census contains 40 rows across the reviewed families; branch state/head advanced from `16/8` to `18/9`.
- Server-owned WorkOrder content hash is `sha256:b667db99ddcbe19267e604d4333d09f4d2cbd3635daab34b306453bf97b96482`. Server-owned Run manifest hash is `sha256:c74bea341813166132f42b6398356a23aaf4785dfdb8e77a75efad5597473cea`; the live runner now pins that exact value.
- Independent-process replay returned `admission_replayed=true`, claimed/delivered/released/terminalized `0/0/0/0`, every target row delta 0, 236 protected tables unchanged, old revision/Run unchanged and prohibited effects all 0.
- Sequence-9 offline preflight passed against exact bundle hash `sha256:458b0e58d93974e3a09b63247bac675d26deef5fdafb111a6eae66177a3b178e`, `ecs.g6.large`, `2 CPU / 8192 MiB`, 30-minute maximum, two-Job/¥50 source ceiling and controller policy hash `6566a47ee9c07ce6a75c9aeedcbc721d299ae52e7620bbbf91e14564b04220d8`. Existing Attempt, cloud call and database-write counts were 0.
- The consumed named-local authorization value was reset to `null`. The paid authorization remains `null`; no STS, cloud read, provider write, `CreateJob`, capability change, Attempt, ExperimentResult, EvidenceCandidate or REU was created.
- Next three actions: (1) obtain separate authorization for one fresh controller STS and one read-only `GetImage`; (2) pass exact image binding and credential cleanup with `CreateJob=0`; (3) only then review a new dated maximum-two-Job and monetary ceiling for normal durable execution.

## 2026-08-02 — durable successor read-only image gate closure

- The owner explicitly authorized one fresh controller STS, one production `GetImage` against the pinned sequence-9 Run/Bundle and complete credential cleanup. Capability changes, provider writes, `CreateJob`, database/scientific/evidence writes and paid runtime were excluded.
- The Cloud Shell `AssumeRole` request used `DurationSeconds=3600` and an inline session policy containing only `paiimage:GetImage` on `*`. The returned credential passed six-key, temporary-identity, exact controller-role, current policy-hash, mode-`0600` and 57-whole-minute local checks without printing secret values.
- Pre-call and post-call offline gates both passed for Run `ef_run_v2_t132_m7_l1_durable_two_cell_successor_v9_1`, manifest hash `sha256:c74bea341813166132f42b6398356a23aaf4785dfdb8e77a75efad5597473cea` and Bundle hash `sha256:458b0e58d93974e3a09b63247bac675d26deef5fdafb111a6eae66177a3b178e`; Attempt count stayed 0.
...
## 2026-08-02 — durable sequence-9 two-cell completion

- One fresh 3,600-second controller STS passed the six-key, mode-`0600`, temporary identity, exact role/current-policy and 59-minute checks. Its session policy allowed only the normal window's exact controller operations. The execute process retained session id `88005`; no second execution process was started.
- The normal `execute` path passed its fresh image gate with request hash `5de0842b5fc3d22d7f99df5da35a5b2fa3cb3b367eee56e8ea3b7bbbf274f549`, created exactly two Attempts and called `CreateJob` exactly twice. Submit commands each succeeded on attempt 1.
- Jobs `dlcxhi03uhwc593e` and `dlcybgs96jh2qj75` independently re-read as `Succeeded`. Provider duration values were 123/120 seconds; running intervals were approximately 8/10 seconds. The corresponding terminal read RequestIds were `019FC108-06C2-5AA8-8E7C-FD8F107E9EF2` and `019FC108-07C6-52C9-9752-9C09260466FA`.
- Both durable Attempts terminalized `succeeded / real_provider_succeeded`. Each has exactly three successful commands (`submit`, `reconcile`, `collect`), one collected collection attempt and one `diagnostic_only` output. Output hashes are `sha256:5da4e1da1c2191380c8489079b9292b0a558692b1cd813798aa645d7c745f883` and `sha256:c703fc917dc2216a2fff3b342113e0b98e4a1d36847f10badda28845d1ed1141`.
- Pack B deltas were Payload/Attempt/Event/Command/Collection/Output `2/2/10/6/2/2`. Built-in exact replay added 0 rows and no Job; 236 protected tables remained unchanged. ExperimentResult, ScientificValidationReport, EvidenceCandidate, scientific evidence and capability persistence counts remained 0.
