...
- The payload materializer now consumes immutable ExecutionBundle code/mirror refs, digests and byte sizes. It rejects non-internal/non-directory/non-content-addressed OSS refs, cross-region/bucket drift, unordered mirror ordinals, unsafe output segments, nested/non-canonical mounts and commands that do not execute from the exact code mount.
