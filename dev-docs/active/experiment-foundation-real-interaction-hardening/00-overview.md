...
- Current focus: Phase 8 UI-driven full-flow smoke landed via T-110 S5. T-106 now imports T-132 M7-I3 offline/default-off evidence run `t132-m7-offline-20260724-v2` (supersedes `t132-m7-offline-20260723-v1` after the 2026-07-24 independent review fixed the reconcile watchdog to wall-clock) from `.ai/.tmp/experiment-foundation-productization/t132-m7-offline-20260724-v2/summary.json`; T-132 remains the sole provider implementation owner. The import closes the M7-15 implementation handoff only. T-106 remains open because the separately authorized true external canary has not run.
...
- [ ] Provider-specific true external canary can verify real connectivity and the minimum real external flow when credentials, environment, budget, and cleanup approval are present. Implementation authority is T-132 M7; T-106 closes the acceptance item only by verifying the T-132 M7 verdict and redaction/cleanup contract.
