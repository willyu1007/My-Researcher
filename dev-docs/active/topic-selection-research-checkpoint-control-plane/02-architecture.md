...
The disposition contract is a precondition, not an arena feature: N4/N6 output schemas, gates, loopbacks, and terminal semantics must admit the two-layer contract as successful outputs — set-level `none_viable`/`evidence_expansion_required`/`reframe_required` and candidate-level `parked`/`dropped`, each carrying evidence refs, per-direction rejection reasons, confidence, and reopening conditions — before or with the first arena slice. Routing of set-level outcomes belongs to the checkpoint, never to the generating agent. Without this legal exit, any critic role is forced to repair fatal findings into an advancing answer.
