...
37. **Next authorization boundary — read-only cloud:** obtain a fresh controller STS, perform exactly one `GetImage` comparison against the pinned sequence-9 Run/Bundle and remove every credential copy. This gate authorizes no capability change, `CreateJob`, provider write or paid runtime. Only after it passes may a dated two-Job/cost ceiling be proposed separately.
