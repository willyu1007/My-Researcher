...
10. **Blocked by two final zero-write preflights 2026-07-28 — executable lineage apply:** named-local has exactly one WorkOrder revision and one Run, both v1 simulation lineage; the RunRecipe has null `executionBundleRevisionId/Hash`. D-23 forbids dispatching or trust-upgrading that immutable Run. The owner then authorized the proposed 40-row same-branch successor apply, but the exact P313 ValidationCycle is already `completed` with immutable `control_flow_validated_no_paper_evidence` closure, so T1 would reject before its first write with `CYCLE_ALREADY_CLOSED`. Do not reopen, mutate or bypass that Cycle/closure. The corrected path requires one separately authorized M7-L1 ValidationCycle under the same active P313 ImplementationProject, one exact complete trace/admission, and a new Cycle-local `ragperf-primary` branch whose first WorkOrder v2 drains T1-T4. Expected maximum is 44 new rows; replay must create zero rows and the historical Cycle/closure/branch/Run plus scientific/evidence/provider-control families must remain unchanged.
11. Direct commits to `main`, without a PR, are the owner-confirmed delivery strategy; do not rewrite or force-update history.
12. **Authorization recorded 2026-07-28 but not consumed:** `M7-L1 authorized: 2026-07-28, ceiling ¥50, 2 jobs`. Immediately before the first `CreateJob`, still require a fresh valid controller STS supplied outside the repository and repeat the read-only `GetImage` comparison if the live window is no longer immediate. Keep both capabilities false until the executable lineage and all pre-submit gates pass.
