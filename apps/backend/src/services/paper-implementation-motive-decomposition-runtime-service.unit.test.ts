...
test('motive decomposition runtime rejects missing or inactive implementation project before provider calls', async () => {
  const missingProject = serviceFixture(undefined, null);
  await assert.rejects(
    () => missingProject.service.runDraftAssertionCandidates(PROJECT_ID, providerRequest()),
  ...
  await assert.rejects(
    () => inactiveProject.service.runDraftAssertionCandidates(PROJECT_ID, providerRequest()),
    (error: unknown) => error instanceof AppError
