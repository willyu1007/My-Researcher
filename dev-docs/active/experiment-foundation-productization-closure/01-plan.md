...
29. **Paused before STS 2026-07-30 — sequence-8 paid verification:** the owner granted the exact two-Job/¥50 window and the zero-cloud gate passed with exact sequence-8 identity and Attempts 0. Cloud Shell replacement-session creation then failed on missing `cloudshell:CreateSession` for `user_0002`; the STS command never executed and `GetImage`/`CreateJob`/provider writes/database writes/Jobs/cost remained 0. Owner/admin must grant only the Cloud Shell bootstrap permission, after which the fresh STS gate may be repeated. If the dated window has expired, renew authorization before invoking the runner.
30. Before marking T-132 `done`, resolve the four non-M7 audit remainders: EF-P06 product candidate promotion, EF-P14 bound bootstrap/legacy-null handling, EF-P15 standalone attachment/revalidation, and EF-P21 semantic retrieval (already deferred by D-24). Each needs either implementation evidence or a named follow-up task; no silent scope loss.
...
10. [ ] Continue the evidence-driven provider compatibility loop. Sequences 3-7 exhausted separately authorized bounded windows without Job creation; sequence 7 proved safe top-level SDK error identity and the repeated provider response `src property must be a valid json object`. Offline run `dbg-20260729-151747-2ddb` now proves both exact sequence-7 final wire bodies are byte/semantic-identical to their SDK model maps, contain zero recursive `src` keys, retain four string-typed object-parseable DataSource Options and omit `AssumeRoleFor`; cloud/database writes were 0. SDK serialization is ruled out, but provider causality remains unresolved. Next compare against the network-aborted console serializer or design a separately authorized minimal no-DataSources provider control; do not create sequence 8 or propose a request fix from the error wording alone.
11. [ ] Complete the personal-use landing: make the exact Run/Attempt/collection/replay state inspectable through the existing desktop workflow and prove a second invocation is safe and comprehensible to the sole maintainer.
...
15. [ ] Resume the owner-authorized sequence-8 paid verification after an owner/admin grants RAM user `user_0002` the narrow Cloud Shell session permission required for `cloudshell:CreateSession`. The 2026-07-30 two-Job/¥50 authorization has not been consumed: zero-cloud preflight passed, but the replacement Cloud Shell session failed before STS generation, so `GetImage`/`CreateJob`/provider writes/database writes/Jobs/cost remained 0. After permission repair, require another fresh STS, retain the active whitelist-only error/wire observers, invoke the runner once and stop at the call ceiling. If execution moves beyond 2026-07-30, obtain a new dated authorization first.
