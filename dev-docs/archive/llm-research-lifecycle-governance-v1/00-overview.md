...
## Status
- State: done
- Next step: 迁移到 `dev-docs/archive/llm-research-lifecycle-governance-v1` 并完成治理同步。
...
- [x] 8 个模块有明确职责边界和依赖方向。
- [x] 4 个阶段具备 entry/exit 门禁定义。
- [x] 模块 4 到 7 形成统一版本主线（含 lineage 字段与冻结策略）。
- [x] 并行线程场景下采用快照冻结（`SP-partial`/`SP-full`）并具备兼容性约束。
