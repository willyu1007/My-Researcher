...
9. **Offline implementation complete; named-local migration + freeze authorization pending:** the reviewed v2 authoring manifest, restart-safe six-row apply, exact replay logic and two-cell default-off same-payload preview are implemented. Disposable PostgreSQL run `t132-m7-bundle-freeze-20260727-v6` passed M7-01..M7-15. Named-local retains v1-only bundle schema CHECKs, so the additive v1/v2 discriminator-binding migration `20260727170000_enable_execution_bundle_schema_v2` must be separately authorized and applied before the six-row freeze; named-local bundle rows remain zero. After apply, rerun the same importer for zero-new exact replay, then perform a fresh read-only `GetImage` comparison immediately before live submission.
