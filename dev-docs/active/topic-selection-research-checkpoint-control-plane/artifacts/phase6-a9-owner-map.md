...
| Current human authorization | Exact current-packet HumanConfirmNeed, N2 constraint-profile acceptance, N5 option A selection, and N7 question-checkpoint `loopback` recorded; revised ResearchSlice direction is pending researcher choice |
...
| Missing nearest work | [Know Before You Fetch](https://arxiv.org/abs/2606.29959) is near-isomorphic to calibrated graded retrieval-budget allocation; [When Should Active RAG Retrieve?](https://arxiv.org/abs/2607.24010) makes realized-budget, harm, and trigger-cost controls explicit. Neither source is yet product-owned evidence for this TitleCard. |
...
The two evidence decisions above are complete and idempotently keyed, both candidate arenas are materialized, and the selected positive candidate is a human-confirmed ValidatedNeed with a decided gap checkpoint. Preserve the negative arena as a non-advancing control. N2 through N7 remain durable, but the exact N7 checkpoint is now non-advancing and loops back to `research_slice`. Resume from the product research status, not directly from N8: first obtain the researcher's route choice, then refresh nearest-neighbor evidence through canonical APIs and produce a substantively revised ResearchSlice/current question lineage. Do not import external sources, invoke a provider, or treat the advisory web review as product authority without separate authorization.
