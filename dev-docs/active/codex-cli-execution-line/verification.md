...
| Cross-model review (gpt-6-astra, 2026-09-09): six findings, all confirmed, all fixed. | Each claim below was reproduced or read against the code before being acted on; the live checks were re-run under the opt-in afterwards. | fixed (2026-09-09) | Live 4/4 after the fixes, which also proves the `-c` override form of the url server on a real Codex. |
| A prompt too large for a child that exits early does not crash the backend. | Real subprocess: 1 MiB on stdin to a child that exits immediately. | fixed (2026-09-09) | Reproduced first as `UNCAUGHT_EXCEPTION EPIPE` with the original pattern. `stdin` now has an error listener; the exit still arrives on `close` and becomes a failed result with its trace. |
| A timeout settles even if the child never exits, so the attempt's handle is released. | Real subprocess: a child that sleeps 60s under a 300ms timeout. | fixed (2026-09-09) | Previously the timer only sent SIGTERM and the promise waited for `close`, which never came if the child ignored the signal or a descendant held the pipes — and the orchestrator's `finally` never ran. Now the promise settles on the timer with whatever arrived, the process group gets SIGTERM, and SIGKILL follows after a grace period. |
| Concurrent invocations cannot overwrite each other's configuration. | Read against the runner; then the live checks under the new shape. | fixed (2026-09-09) | Every run wrote the same `<home>/config.toml`. Now nothing is written to the home at all: the approval policy and the MCP servers travel as `-c` overrides, and the per-run scratch directory holds only the schema file. This also deleted the config generator outright. |
| Multibyte output survives pipe chunk boundaries. | Read against the runner. | fixed (2026-09-09) | Per-chunk `toString('utf8')` replaced a character split across reads with U+FFFD while leaving the JSON valid, silently corrupting non-ASCII evidence and traces. Both streams now decode with `setEncoding('utf8')`. |
| Concurrent tool calls cannot both be served against a budget that covers one. | `Promise.all` of two one-unit reads against a budget of one. | fixed (2026-09-09) | Reproduced first as `ok ok, reads_used = 2`. The reservation now happens before the handler is awaited and is refunded if the handler throws; a concurrent test and a refund test pin both. |
| A codex_cli invocation blocked before any run reports the blocker instead of failing audit validation. | The prompt-quality fixture (forbidden secret in the prompt) against an admitted codex_cli profile. | fixed (2026-09-09) | `sourceKindForExecutionMode` fell through to `provider_response` for the new mode, producing a provenance with no model identity that the audit schema rejected as `INTERNAL_ERROR`. The mode now maps to its own source kind; the schema branch requires only the identity known before a run, with the run fields set by the success path; and the configuration check moved ahead of the gates. |
