...
The T-151 orchestrator has a `codex_cli` branch and a runner outcome with trace provenance. Shipped profiles do not yet admit that line, v1b harness modes exclude it, and canary-local injection/admission does not wire the application. T-152 completed its App Server transport at `a5263023`; App Server is the default and the runner contract remains the consumer boundary. Its canaries are foundation evidence, not T-153 product-node evidence.
