...
- Implementation checkpoint (2026-08-30): complete. Newly admitted role executions use `TopicSelectionResearchArenaRoleExecution@v2` and bind the exact succeeded product invocation-audit artifact ref/hash plus execution-provenance hash; admission checks the snapshot, workflow/node/attempt, `multi_agent_debate` acceptance mode, and output hash before persistence. Transcript v2 names those exact execution identities, and synthesis rejects missing, duplicated, legacy-only, or forged first passes plus termination/disposition drift. Existing v1 rows remain readable only as `legacy_unverified` with null audit identity and cannot satisfy v2 synthesis, so the historical single-main-agent replay and task-only `fork_turns=none` calibration cannot be relabeled as verified product execution. `NeedCandidate` now owns one stable semantic-group hash and one typed `currentArenaAdvisory` projection containing the session/synthesis identity, support-only disposition, rationale, drop reason, reopening conditions, and selected-against candidate. Session synthesis and all candidate projections commit atomically; the arena never changes formal decision, lifecycle, review, or freshness state and exposes no checkpoint, HumanConfirmNeed, AcceptedRisk, promotion, bridge, or fork writer. Shared contracts pass 436/436, the focused backend set passes 161/161, real PostgreSQL repository replay passes 1/1, and the full backend suite on a freshly migrated disposable database reports 2,850 tests: 2,780 passed, 70 conditional skips, and 0 failures. All 80 migrations replay from empty with no Prisma drift; DB context, OpenAPI parse, backend/root typechecks, and diff checks pass. The full-suite audit also repaired a legacy v1c compatibility defect: Prisma reads now preserve an absent optional `risk_finding_refs` field instead of injecting a hash-visible `undefined` key. Phase 9C remains a separate, unauthorized review boundary.
