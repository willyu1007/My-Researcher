...
- 归属本包的 `assertHumanRunBinding` frontier-guard 缺陷由 T-128（`topic-selection-product-readiness-closure`）在其 W-05 Commit-3 Bugbot review 中**顺修并单独提交 `ad1aa8c4`**（非本包 session 改动，此处补交叉引用留痕以补全归属 changelog）：`topic-selection-v1b-controller.ts` `assertHumanRunBinding` 的 frontier 检查含 `&& !humanNode?.latest` 短路 → human node 一旦 admit（`latest` 存在），即便 frontier 已前进，迟到/重复的同-run human 写入仍被放行 → 落 stale attempt 污染 run。修法：删 `&& !humanNode?.latest`，frontier 严格只认 `next_node_id`（retry/loopback 因 next_node_id 仍指向自身而通过；coordinator `getRunState` 投影由 lastCompleted 的 invoke_next 边即时派生 next_node_id）+ 更新注释 + integration coordinator e2e 加 frontier 回归断言（late 同-run + 正确 N4 target → 409 `VERSION_CONFLICT` "is not awaiting"）。详见 `dev-docs/archive/topic-selection-product-readiness-closure/04-verification.md` 2026-06-30 段。
