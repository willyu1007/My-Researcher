...
- Role-specific retrieval and EvidencePacket resolution are separated by a deliberate materialization boundary. Product retrieval may discover literature outside the one current reviewed EvidenceMap and records that divergence in SearchRun provenance, but the preparation result is `requires_evidence_materialization` until a current reviewed claim-bearing EvidenceUnit with a resolvable locator exists. Raw RAG hits, result summaries, and similarity scores never substitute for EvidenceMap review or keyed source resolution.
