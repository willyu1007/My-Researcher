...
- **做完整运行时**（非半成品）——要让"候选弱→升级 debate→正常继续"真正可走,必须 a–i 全做:
  - 原语:生成型任务,倾向 divergent_loop（explorer 探不同 framing → critic 挑战 → arbiter 综合子集）;亦可 bounded_sequence（复用 v1c N2 / v1b N8 范式）。**3.1 设计定（2026-06-19，用户确认 + D-T127-02 锁定）：divergent_loop（3 角色 explorer→critic→arbiter）**——更贴合 N6 生成型探索任务;模板镜像 V1A divergent_loop debate（`topic-selection-need-discovery-debate-loop-service.ts` + `createTopicSelectionV1aGenerateNeedCandidateDebateScenarioContract`）,非 N8 bounded_sequence;debate 本体复用共享 debate-core runLoop（注入 N6 divergent_loop strategy,零 core 改动）。
