...
export class PaperImplementationFeasibilityPlanningRuntimeService {
  ...
  private fixtureOutputForMode(
    ...
    if (request.execution_mode === 'mocked_llm') {
      return request.mocked_role_outputs?.[PAPER_IMPLEMENTATION_FEASIBILITY_PLANNING_ROLE_SLOT_ID] ?? null;
    }
    if (request.execution_mode === 'codex_assisted') {
      return request.codex_role_outputs?.[PAPER_IMPLEMENTATION_FEASIBILITY_PLANNING_ROLE_SLOT_ID] ?? null;
    }
    return null;
  }
