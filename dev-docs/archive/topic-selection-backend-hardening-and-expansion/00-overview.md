...
- **残留移交登记（→ T-128 `topic-selection-product-readiness-closure`，2026-06-25 收尾）**：本包工程项全部完成；所有**延期/外部门控/孤儿**义务移交后继包，逐项已被 T-128 `00-overview` 显式认领（无孤儿）——① N8/N6 真标定 + 翻门（record-and-defer，外部语料硬门，工程造不出）→ **T-128 W-17**；② D5 HumanOverride 写面 + Trace-snapshot 抽屉（D5 延期）→ **T-128 W-15**；③ **P-01 压缩恢复**——T-127 D3「topic-selection 侧回归共同确认」义务在本包收口时未碰、曾成孤儿 → **T-128 W-11**（跨 T-124/T-088 协调）；④ W-09 dormant `provider_llm` debate plans（预接、tripwire 门控）→ **T-128 W-14**；⑤ W-07 step-f prompt 骨架（产品正文延期）→ **T-128 Phase 1 prompt 撰写 + Phase 5 语料耦合 debate 正文**。**归档**：本包**暂留 `dev-docs/active/`**，状态 `done`，待与 T-128 收口时**一并归档**（用户定，2026-06-25）。
- Task ID: `T-127`
