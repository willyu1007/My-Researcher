...
4. **Compatibility implementation completed and verified 2026-07-26:** the shared exact request/profile schemas, materializer, official-SDK wire map, redacted manifest/hash and negative tests now bind exact `DataSources`, standard `Envs` and runtime-role `CredentialConfig`. No cloud operation was executed. Remaining C prerequisites are the actual official `ImageUri`/acceptable immutable image identity and separate PAI/DLC mount-service authorization verification; the console display label alone is not sufficient.
5. Complete D preparation: create digest/size manifests for the content-addressed workload directory and the two ordered SciFact mirror directories, upload them to exact `input/` prefixes only in a separately confirmed window, and verify the final object refs without enabling `CreateJob`.
6. Direct commits to `main`, without a PR, are the owner-confirmed delivery strategy; do not rewrite or force-update history.
7. Immediately before the first `CreateJob`, require a fresh short-lived controller STS supplied outside the repository and the exact in-session authorization string with date, ¥50 ceiling and two-job limit. Keep both capabilities false until then.
8. Run M7-L1 as diagnostic-only and require `real_provider_canary_passed`, terminal/cleanup verification, zero duplicate jobs and zero ExperimentResult/EvidenceCandidate/REU. M7-L2 remains a separately authorized scientific closure.
9. Before marking T-132 `done`, resolve the four non-M7 audit remainders: EF-P06 product candidate promotion, EF-P14 bound bootstrap/legacy-null handling, EF-P15 standalone attachment/revalidation, and EF-P21 semantic retrieval (already deferred by D-24). Each needs either implementation evidence or a named follow-up task; no silent scope loss.
...
Ownership is singular: T-132 implements M7 and T-106 consumes its final redacted evidence. The existing acknowledged Run is immutable simulation-only history (`materialize-only` RunRecipe, `simulation_*` TaskSpec outputs and simulation-only DB CHECKs), so M7 cannot dispatch it or upgrade its trust. D-23 requires a new PI WorkOrder revision bound to an exact typed `ExecutionBundleV2`; the normal T1-T4 spine then creates a new executable Run/head acknowledgement.
