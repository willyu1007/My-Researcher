...
12. **Authorization recorded 2026-07-28 but not consumed:** `M7-L1 authorized: 2026-07-28, ceiling ¥50, 2 jobs`. Controller policy v2 activation and the production read-only `image-preflight` have passed. Immediately before the first `CreateJob`, still require a controller STS supplied outside the repository with ≥55 minutes remaining at execute start. Keep both capabilities false until the execute command enters its bounded live window.
13. Run M7-L1 as diagnostic-only and require `real_provider_canary_passed`, terminal/cleanup verification, zero duplicate jobs and zero ExperimentResult/EvidenceCandidate/REU. M7-L2 remains a separately authorized scientific closure and requires OCI/content-digest image identity.
14. Before marking T-132 `done`, resolve the four non-M7 audit remainders: EF-P06 product candidate promotion, EF-P14 bound bootstrap/legacy-null handling, EF-P15 standalone attachment/revalidation, and EF-P21 semantic retrieval (already deferred by D-24). Each needs either implementation evidence or a named follow-up task; no silent scope loss.
...
7. [ ] At the owner-confirmed paid window, obtain/reuse a controller STS with ≥55 minutes remaining at execute start and execute/collect the exact two-job diagnostic under the ¥50 ceiling; then run exact replay and protected/scientific negative-space verification.
8. [ ] Complete the personal-use landing: make the exact Run/Attempt/collection/replay state inspectable through the existing desktop workflow and prove a second invocation is safe and comprehensible to the sole maintainer.
