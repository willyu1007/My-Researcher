...
## Acceptance Criteria
- [ ] W-01:孤儿 run 可自动恢复(启动+入队双路径),并发 enqueue 在 DB 层互斥;负例测试齐(崩溃模拟/并发注入)。
- [ ] W-02:GROBID 调用有超时/有界重试/断路器;健康检查接入执行前置;部署约定成文。
- [ ] W-03:ANN 索引落地或以实测数据证明当前规模无需(留痕);检索超时取消 DB 查询;关键参数入 settings。
- [ ] W-04:embedding cost_usd 真实计费;超限文献分批不整篇失败。
