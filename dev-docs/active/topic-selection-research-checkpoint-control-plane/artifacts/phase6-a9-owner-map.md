...
| Current N5 frontier | Next node `topic-selection.v1b.select-research-slice.v1`; zero attempts. Strict-human selection must bind the exact current option-set authority/handoff before any ResearchSlice or question candidate exists. |
| Current-lineage boundary proof | Current N2-N4 owners exist and bind current ValidatedNeed/bundle/intake/evidence lineage. N5 remains unwritten, so no current ResearchSlice, question/package/v1c owner, HumanPromotionDecision, PromotionDecision, bridge, PaperProject, provider call, or corpus mutation occurred. |
| Current v1c frontier | Machine Gate remains independently `ready_for_human_decision`, but the old question/package/v1c/promotion chain is stale relative to the repaired gap lineage and must not be retried. The earliest unsatisfied point is strict-human N5 on the current run; after selection, rebuild N6/N7 and present the newly snapshot-bound question confirmation. |

## Resume guard

The historical evidence decisions remain durable, both earlier candidate arenas remain readable, and their human-confirmed ValidatedNeed is preserved as provenance, but the old gap authority is upstream-stale relative to refreshed evidence checkpoint `research_checkpoint_19a6227d-3849-4d0e-ab6f-9e89e072b457`. Preserve the negative arena as a non-advancing control. The bounded two-paper acquisition, parsing, curation, and EvidenceMap refresh are complete, and downloader `max_byte_size` is restored to `1024`. Current-evidence gap checkpoint `research_checkpoint_de57273c-4b2e-4820-9f98-898920a6fe63` is decided `advance`; A is the human-confirmed source of current ValidatedNeed `validated_need_a58766a3-d482-4dc3-9b0b-cdeac9e3d095`, while C remains the unselected viable alternative. Current v1b bundle `v1b_input_bundle_fc913271-74bb-4359-b19b-0d4216a609bc` and run `workflow_run_t147_a9_pos_v1b_current_gap_v1` are durable; N1-N4 are admitted and current option set `research_slice_option_set_45f3d8bc-9a9e-4142-b905-f5d9c2691b2a` is frozen. Resume only by presenting A/B/C from that exact set and recording one N5 strict-human selection after explicit researcher acceptance. Do not retry old promotion checkpoint `research_checkpoint_35406533-fac3-4cdb-b6d9-29296ca66c63` or its stale downstream lineage. Stop again at the new question checkpoint; do not invoke a provider, advance N8, or create a bridge without the corresponding new authorization boundary. The configured raw-file root is transient; if its OA PDFs disappear, obtain exact authorization and reacquire only those two assets through a new bounded dry-run rather than overwriting storage settings.
