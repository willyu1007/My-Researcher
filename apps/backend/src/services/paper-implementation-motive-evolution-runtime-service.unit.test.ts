...
test('motive evolution runtime rejects missing or inactive implementation project before provider calls', async () => {
  const missingProject = serviceFixture(undefined, undefined, null);
  await assert.rejects(
    () => missingProject.service.runEvolutionDecisionSupport(PROJECT_ID, providerRequest()),
  ...
  const inactiveProject = serviceFixture(undefined, undefined, implementationProjectFixture('archived'));
  await assert.rejects(
    () => inactiveProject.service.runEvolutionDecisionSupport(PROJECT_ID, providerRequest()),
    (error: unknown) => error instanceof AppError
