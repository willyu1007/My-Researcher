...
import { InMemoryPaperImplementationValidationCycleClosureV2Repository } from './repositories/paper-implementation-validation-cycle-closure-v2.repository.js';
...
import { PrismaExperimentFoundationExplorationSpecV2Repository } from './repositories/prisma/prisma-experiment-foundation-exploration-spec-v2-repository.js';
import { PrismaExperimentFoundationScientificValidationV2Repository } from './repositories/prisma/prisma-experiment-foundation-scientific-validation-v2-repository.js';
...
import { PrismaPaperImplementationEvidenceV2Repository } from './repositories/prisma/prisma-paper-implementation-evidence-v2-repository.js';
...
import { PrismaPaperImplementationValidationCycleClosureV2Repository } from './repositories/prisma/prisma-paper-implementation-validation-cycle-closure-v2-repository.js';
import { PrismaPaperImplementationCycleReadinessV2Repository } from './repositories/prisma/prisma-paper-implementation-cycle-readiness-v2-repository.js';
...
import { registerTopicSelectionResearchArenaCalibrationRoutes } from './routes/topic-selection-research-arena-calibration-routes.js';
...
import type { ExperimentFoundationExplorationSpecV2Repository } from './repositories/experiment-foundation-exploration-spec-v2.repository.js';
...
import { TopicSelectionResearchArenaCalibrationService } from './services/topic-selection-research-arena-calibration-service.js';
...
export type BuildAppOptions = {
  ...
  experimentFoundationExplorationSpecV2Repository?: ExperimentFoundationExplorationSpecV2Repository;
  ...
  experimentFoundationV2ExplorationSpecEnabled?: () => boolean;
...
export function buildApp(options: BuildAppOptions = {}): FastifyInstance {
  ...
  const experimentV2ExplorationSpecEnabled =
    options.experimentFoundationV2ExplorationSpecEnabled?.()
    ?? isExperimentFoundationV2ExplorationSpecEnabled();
  ...
  const experimentFoundationExplorationSpecV2Repository =
    options.experimentFoundationExplorationSpecV2Repository
    ?? createExperimentFoundationExplorationSpecV2Repository(
      storeConfig.experimentFoundationStrategy,
    );
  ...
  const paperImplementationCycleReadinessV2Repository =
  ...
  const paperImplementationValidationCycleClosureV2Repository =
    ...
    ?? createPaperImplementationValidationCycleClosureV2Repository(
      storeConfig.paperImplementationStrategy,
      paperImplementationCycleReadinessV2Repository,
    );
  const paperImplementationEvidenceV2Repository =
    options.paperImplementationEvidenceV2Repository
    ?? createPaperImplementationEvidenceV2Repository(storeConfig.paperImplementationStrategy);
  const experimentFoundationScientificValidationV2Repository =
    options.experimentFoundationScientificValidationV2Repository
    ?? createExperimentFoundationScientificValidationV2Repository(
      storeConfig.experimentFoundationStrategy,
    );
  const hasDefaultDurableExperimentV2Composition =
    ...
    && options.experimentFoundationExplorationSpecV2Repository === undefined
    ...
    && options.paperImplementationEvidenceV2Repository === undefined
  ...
  const experimentFoundationExplorationSpecV2Service =
    new ExperimentFoundationExplorationSpecV2Service(
      experimentFoundationExplorationSpecV2Repository,
      {
        enabled: () => (
          hasDefaultDurableExperimentV2Composition
          && experimentV2ExplorationSpecEnabled
        ),
      },
    );
  const experimentFoundationExplorationSpecV2Controller =
    new ExperimentFoundationExplorationSpecV2Controller(
      experimentFoundationExplorationSpecV2Service,
    );
  ...
  const paperImplementationValidationCycleClosureV2Service =
    new PaperImplementationValidationCycleClosureV2Service({
      ...
      enabled: () => (
        validationCycleClosureV2Enabled
        && (
          hasDefaultDurableExperimentV2Composition
          || options.paperImplementationValidationCycleClosureV2Repository !== undefined
        )
      ),
    });
  const paperImplementationCycleReadinessV2Service =
  ...
  const experimentFoundationScientificValidationV2Service =
  ...
  const paperImplementationProjectionFeedV2Consumer =
  ...
  app.decorate('experimentV2IntegrationRelayService', experimentV2IntegrationRelayService);
  ...
  const topicSelectionResearchArenaCalibrationController =
    new TopicSelectionResearchArenaCalibrationController(
      new TopicSelectionResearchArenaCalibrationService({
  ...
  // D1 coordinator: only runtime slot services + own repository + project
  // preflight + the narrow decision-queue writer — structurally no domain
  // authority repositories. The writer is deliberately wrapped to an
  // enqueue-only object literal (W4) so the coordinator's runtime surface
  // cannot reach the rest of the harness repository.
  const paperImplementationRunCoordinatorService = new PaperImplementationRunCoordinatorService({
  ...
  const paperImplementationEvidenceBoardHandoffService =
    new PaperImplementationEvidenceBoardHandoffService({
      projectRepository: paperImplementationRepository,
      motiveRepository: paperImplementationMotiveRepository,
      evidenceMapRepository: topicSelectionEvidenceMapRepository,
  ...
  app.register(async (instance) => {
    ...
    await registerTopicSelectionResearchArenaCalibrationRoutes(
      instance,
      topicSelectionResearchArenaCalibrationController,
    );
...
function createTopicSelectionResearchCheckpointRepository(
  strategy: RepositoryStrategy,
): TopicSelectionResearchCheckpointRepository {
  if (strategy === 'prisma') {
    return new PrismaTopicSelectionResearchCheckpointRepository(getPrismaClient());
  }
...
function createPaperImplementationValidationCycleClosureV2Repository(
  strategy: RepositoryStrategy,
  readinessRepository: PaperImplementationCycleReadinessV2Repository,
): PaperImplementationValidationCycleClosureV2Repository {
  if (strategy === 'prisma') {
    return new PrismaPaperImplementationValidationCycleClosureV2Repository(getPrismaClient());
  }

  return new InMemoryPaperImplementationValidationCycleClosureV2Repository({
    readinessRepository,
  });
}

function createPaperImplementationCycleReadinessV2Repository(
  strategy: RepositoryStrategy,
): PaperImplementationCycleReadinessV2Repository {
  if (strategy === 'prisma') {
...
function createPaperImplementationEvidenceV2Repository(
  ...
  if (strategy === 'prisma') {
    return new PrismaPaperImplementationEvidenceV2Repository(getPrismaClient());
  }
  return new InMemoryPaperImplementationEvidenceV2Repository();
}
...
function createExperimentFoundationExplorationSpecV2Repository(
  strategy: RepositoryStrategy,
): ExperimentFoundationExplorationSpecV2Repository {
  if (strategy === 'prisma') {
    return new PrismaExperimentFoundationExplorationSpecV2Repository(getPrismaClient());
  }
  return new InMemoryExperimentFoundationExplorationSpecV2Repository();
}
...
function createExperimentFoundationScientificValidationV2Repository(
  strategy: RepositoryStrategy,
): ExperimentFoundationScientificValidationV2Repository {
  if (strategy === 'prisma') {
    return new PrismaExperimentFoundationScientificValidationV2Repository(getPrismaClient());
  }
  return new InMemoryExperimentFoundationScientificValidationV2Repository();
}
...
function createPaperImplementationCoordinatorRepository(
  strategy: RepositoryStrategy,
): PaperImplementationCoordinatorRepository {
  if (strategy === 'prisma') {
    const prisma = getPrismaClient();
    return new PrismaPaperImplementationCoordinatorRepository(prisma);
  }
...
function isExperimentFoundationV2ExplorationSpecEnabled(): boolean {
  return parseExperimentV2BooleanEnvironmentVariable(
...
function assertPaperImplementationExperimentV2CutoverConfig(input: {
  ...
  if (input.explorationSpecEnabled && !input.cutoverCommitted) {
    throw new Error(
      'EXPERIMENT_FOUNDATION_V2_EXPLORATION_SPEC_ENABLED requires '
      + 'PAPER_IMPLEMENTATION_EXPERIMENT_V2_CUTOVER_COMMITTED=true',
    );
  }
}
