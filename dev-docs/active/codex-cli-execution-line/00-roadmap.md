...
| D-9 Which MCP revision does the product server target? | (a) only `2025-06-18`, matching Codex today; (b) `2026-07-28` with backward compatibility; (c) `2026-07-28` only | (c) `2026-07-28` only, no backward compatibility | decided | User, 2026-09-08 | User instruction: target 2026-07-28, compatibility not considered for now | Attempt scoping rides on a server-minted handle passed as a tool argument, not on session or process state, and the server implements `server/discover` with no `initialize` handshake. **Consequence:** no released Codex speaks this revision — 0.153.4 is the latest published build as of 2026-09-08 and negotiates `2025-06-18` — so the tool surface cannot serve Codex until Codex adopts a newer revision. Phase 1 is unaffected; the tool-surface phases inherit an external dependency. Revisit if that wait proves unacceptable. |
...
- [ ] Decisions: D-7 is confirmed; D-1 through D-6, D-8 and D-9 are decided.
...
| No released Codex speaks MCP `2026-07-28`, the revision D-9 targets, so the product tool surface cannot serve Codex until Codex upgrades. | Codex `--version` against the revision its MCP client negotiates. | Phase 1 carries no tool surface and is unaffected; build the server against the current spec so no rework is needed when Codex catches up. | Revisit D-9 and add a compatibility path for the handshake-based revisions. |
