...
- [ ] `select`, `park`, `drop`, `reframe`, and `expand` are explicit outcomes; zero viable candidates is successful when evidence and reopening conditions are recorded; a fork recommendation is recorded as `park` plus a surfaced recommendation.
