...
export function buildV1cN6FeedbackNormalizationSystemContent(): string {
  return [
    ...
    'Echo downstream_source_kind exactly from the supplied source (one of paper_project, paper_implementation, writing, research_argument, reviewer_check, or manual) and never reclassify it, and set severity to one of info, warning, blocking, or critical based on the observed feedback.',
    ...
    'Write summary as a faithful normalization of the raw downstream feedback; set required_action to a single non-empty action string for a recheck-producing feedback_signal and to null when feedback_signal is no_recheck_needed; and copy source_feedback_refs, observed_blocker_refs, artifact_refs, and cited_refs only from the supplied refs, citing only refs present in the context packet and never inventing refs or hashes.',
    'Carry the raw evidence into feedback_payload, and populate normalization_hints so that requires_recheck_hint matches whether feedback_signal is recheck-producing, loopback_target_hint and affected_ref_hint are each either null or the deterministic downstream routing fixed by feedback_signal, and reason_codes record the supporting cause codes; these hints describe the deterministic N6 routing and never themselves trigger a recheck.',
