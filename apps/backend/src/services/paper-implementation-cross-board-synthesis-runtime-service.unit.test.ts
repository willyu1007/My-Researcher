...
test('cross-board synthesis runtime token budget counts message-embedded context once and wires the compression attempt (S2-A)', async () => {
  ...
  const request = {
    ...base,
    run_id: 'cross_board_synthesis_token_budget_run_001',
  ...
  await service.runMergeSplitReuseScenarios(PROJECT_ID, request);
  ...
  await noPackets.service.runMergeSplitReuseScenarios(PROJECT_ID, {
    ...providerRequest(),
    run_id: 'cross_board_synthesis_token_budget_run_002',
  });
