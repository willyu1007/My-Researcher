...
## Execution posture
- Current state: `done`; M0-M7、OQ-01 through OQ-22、Pack A/Pack B、Pack C、agent/API usage-fit、OpenAPI/context、named-local no-evidence Cycle closure、default-off release controls and the exact sequence-9 durable two-cell PAI acceptance are complete. The final live window produced two terminal successful Jobs, two collected `diagnostic_only` outputs and zero-new replay while scientific/evidence/capability promotion remained zero. Current named-local cutover is `true`; admission, simulation, scientific-validation, PI-closure, cloud-preflight and real-provider capabilities are restored to `false`. Desktop/UI, generalized cloud productization and the four transferred audit remainders are outside T-132; T-134 owns the latter.
...
## Current remaining sequence — reconciled 2026-07-28
...
2. **Completed 2026-07-29 — B / RAM PassRole recovery:** both custom policies and separate roles are created and console-verified. Controller policy v3 is default at repository SHA-256 `f6b63cd73a57c6d8cfade1a177681ad4463cbd4d6d0a116e26a40ceee85ed497`; it adds only exact `ram:PassRole` for `acs:ram::1183869713036194:role/pea-m7-canary-runtime`. v1/v2 remain non-default rollback versions. Runtime policy remains v2 at repository SHA-256 `1eb7de00aceacc14817b058291eb4f2e85cdbb4c10ea467c91084a75094b1a4b`, with `ListObjects` restricted to `input` / `input/*` and object read/write statements separated. Controller role `300042892692129613` trusts only owner user `acs:ram::1183869713036194:user/user_0002`; runtime role `300525928077898732` trusts only PAI service `pai.aliyuncs.com`. Each role has exactly its matching custom policy and no cross-attachment.
...
9. **Completed 2026-07-27 — named-local bundle migration/freeze/replay and fresh image comparison:** the reviewed v2 authoring manifest, restart-safe six-row apply, exact replay logic and two-cell default-off same-payload preview are implemented. Disposable PostgreSQL run `t132-m7-bundle-freeze-20260727-v6` passed M7-01..M7-15. After explicit supplemental authorization, additive v1/v2 discriminator-binding migration `20260727170000_enable_execution_bundle_schema_v2` was applied to named-local. r1 created exactly six bundle rows and r2 created zero while exact-reusing all six; both runs proved 244 protected tables unchanged and all cloud/provider/scientific counters zero. A later separate cloud-read authorization completed two read-only `GetImage` operations: all frozen provider-managed identity fields matched and every write counter remained zero. Repeat the comparison if the live submission window is not immediate.
10. **Completed 2026-07-28 — executable lineage apply:** the corrected path created/admitted one new P313 `probe_execution` ValidationCycle, input snapshot and exact complete trace, then created the Cycle-local `ragperf-primary` branch, executable WorkOrder v2, two cells and admission. PI and EF Prisma readback fences were repaired to bind exact v1/v2 content after the first T2 transaction correctly rolled back on the stale v1-only assertion. A separately authorized one-row recovery returned only `pi_experiment_outbox_v2_t132_m7_l1_p313_v1_1` from `terminal` to `pending`, preserved `relayAttemptCount=1`, and added no row. T1-T4 then delivered 3/3 events; 9 pre-existing authorized rows + 35 new rows = exactly 44, replay added zero, 233 protected tables and all historical authority stayed unchanged, and every cloud/provider/capability/`CreateJob`/scientific/evidence counter stayed zero. Durable closure: `artifacts/implementation/23-m7-l1-executable-lineage-closure.md`.
11. Direct commits to `main`, without a PR, are the owner-confirmed delivery strategy; do not rewrite or force-update history.
12. **Paid attempt stopped before submission; authorization consumed zero jobs/cost:** `M7-L1 authorized: 2026-07-28, ceiling ¥50, 2 jobs`. The execute path passed exact offline lineage and performed one read-only `GetImage`, then failed closed before `CreateJob` because the normal persistence path rejected the valid provider-managed redacted manifest v2. Cloud Job, billable compute, Attempt and database writes remained zero.
13. **Completed 2026-07-28 — named-local migration apply:** after separate approval, applied only `20260728140500_enable_real_provider_payload_manifest_v2`. All 250 application tables retained the same 3,370,691 rows and exact primary-key/xmin digest; six Pack B counts stayed `2/2/12/8/2/2`, exact Run Attempt count stayed 0, CHECK readback matched the reviewed simulation-v1/real-provider-v1-v2 contract and offline-preflight passed with zero cloud/database writes.
14. **Paid rejection root cause proven; exact PassRole fix applied and read-only verified.** The hard-fenced client invoked `CreateJob` exactly twice for sequence 3; both RequestIds resolve to PAI error 4001 `NoPermission` while checking the request's RAM role. Gate 2 added only exact `ram:PassRole` for the runtime role. Repository policy/hash and runner pin are updated; owner-confirmed cloud policy v3 is default, v1/v2 remain rollback, canonical policy semantics match and runtime trust is unchanged. A fresh controller STS passed one production `GetImage` read with provider writes/`CreateJob`/database writes all zero, then every temporary STS file was removed. End-to-end provider verification still requires a new successor and separate paid authorization.
15. **Completed 2026-07-29 — sequence-4 post-fix lineage, preparation and bounded paid verification:** the named-local apply created WorkOrder `pi_experiment_revision_v2_t132_m7_l1_passrole_fix_successor_v4_1` and Run `ef_run_v2_t132_m7_l1_passrole_fix_successor_v4_1` under the existing open Cycle/branch. Normal T1-T4 added exactly 40 rows and advanced branch state/head from `6/3` to `8/4`; 236 protected tables, sequence-3 Attempts/ProviderCommands and all prior lineage remained unchanged. In-process and independent-process exact replay both added zero rows. Offline and production image preflights passed. Under the exact two-job/¥50 authorization, the runner then invoked `CreateJob` exactly twice; both crossed the repaired PassRole boundary but returned HTTP 400 `BadRequest`, with OpenAPI self-diagnosis `src property must be a valid json object`. No Job/external ref or billable runtime resulted. Sequence 4 now has 2 immutable prepared Attempts, 2 pending recovery-only submit commands, and zero ExperimentResult/EvidenceCandidate/REU. The paid authorization is exhausted.
16. **Completed 2026-07-29 — no-cloud request-shape diagnosis:** Gate 1 added removable whitelist-only structural observations and exact offline rematerialization. Both sequence-4 requests JSON-round-trip, contain zero `src` keys and show four direct OSS DataSources with no `Options`; 4/4 privacy tests, both typechecks and zero-cloud/zero-write offline-preflight pass. The logged-in PAI form was not submitted; adding an OSS mount showed its provider-owned Advanced Settings default is exactly `{}`. This supports an Options-only compatibility fix while retaining provider-regression uncertainty until live verification.
17. **Completed offline 2026-07-29 — Gate-2 Options-only compatibility fix:** after `APPROVE FIX`, all direct OSS DataSources emit canonical `Options: '{}'`; the shared type/schema requires that exact value, the manifest redaction census and Prisma readback fence agree, and focused tests reject missing/non-canonical Options. The first full gate exposed the stale readback census and failed 8/9 relational tests; after the same-boundary correction, `t132-m7-options-empty-object-20260729-v2` passed shared 12/12, backend 93/93 and relational 9/9. No named-local or cloud write occurred.
18. **Completed 2026-07-29 — immutable sequence-5 named-local verification path:** the guarded runner added one exact Options-fix successor token and truthful WorkOrder intent. Read-only preflight proved branch `8/4`, one exact sequence-4 parent revision/Run, empty sequence-5 prefix and open Cycle. Authorized normal T1-T4 added exactly 40 rows and advanced branch state/head to `10/5`; relay delivered 3/3, 236 protected tables changed 0 and prior lineage stayed immutable. In-process and independent-process exact replay each added zero rows. The live runner is rebound to sequence 5 and its offline-preflight passes with Attempts/cloud/database writes all 0.
19. **Completed 2026-07-29 — sequence-5 read-only production image preflight:** reconnected Cloud Shell and explicitly skipped NAS creation. A newly assumed controller STS was written/downloaded without printing values, moved immediately from Downloads to `/tmp` mode `0600`, and validated for six exact keys, temporary AK prefix, exact role/policy hash and 58 whole minutes remaining. `image-preflight` passed for sequence 5 with image request hash `d74c81437c8ebf215cb052f22b34a883538c8536632edfcdd288f4621c04ef92`; cloud calls 1, provider writes/`CreateJob`/database writes 0. Cloud Shell, `/tmp` and Downloads credential copies were removed.
20. **Completed but rejected 2026-07-29 — sequence-5 paid provider verification:** a new fresh STS passed the exact contract and the hard-fenced runner invoked `CreateJob` exactly twice. Both wire maps contained four `Options` strings, sizes 2,979/2,976 bytes, zero recursive `src` keys and no unknown fields; both still received HTTP 400. OpenAPI self-diagnosis returned the same `src property must be a valid json object` response for both RequestIds. Returned/discovered Jobs, external refs, collection/output/scientific/evidence rows and billable runtime all remained 0. The authorization is exhausted and all credentials are removed.
21. **Completed 2026-07-29 — sequence-6 console-parity role-shape path and read-only image preflight:** Gate 1/2 isolated optional `AssumeRoleFor`; the approved fix emits only exact `RoleArn + RoleType`. Authorized named-local T1-T4 created the immutable sequence-6 WorkOrder/Run with exactly 40 new rows, branch `12/6`, zero-new replay and no Attempt/evidence rows. A separately authorized fresh controller STS passed six-key, `0600`, temporary-AK, exact role/policy and 58-minute checks; one `GetImage` matched the frozen Bundle with request hash `ebcc558c0fd09b771277101942102d3e6d1bdc1dd3ebbec8ca276d98ab554ae5`. Provider writes, `CreateJob`, database writes and billable runtime were 0, and every credential copy was removed.
22. **Completed but unresolved 2026-07-29 — sequence-6 paid provider verification:** the exact action-time authorization opened a maximum two-Job/¥50 window. A fresh controller STS passed six-key, `0600`, temporary-AK, role/policy and 59-minute checks. The runner consumed the two-`CreateJob` ceiling, but repeated exact recovery found no accepted Job or external ref. Both Attempts terminalized `failed / real_provider_cleanup_unverified`; both submit commands terminalized after 12 passes with `REAL_PROVIDER_RECOVERY_NOT_FOUND`. Collection/output/scientific/evidence rows and observed billable runtime remained 0. The transport's accepted-response-loss policy discarded the synchronous SDK exception, so the custom-role-shape hypothesis is not provider-resolved.
23. **Completed offline 2026-07-29 — no-cloud SDK-error observation gate:** after `APPROVE INSTRUMENTATION`, Debug Mode run `dbg-20260729-142414-8438` added a removable observer at the bounded `CreateJob` catch boundary. It emits only type-fenced status/code/RequestId and source labels, never reads getters, message, stack, request/response bodies, non-whitelisted headers, role/account, URI, command, environment or credential values, and rethrows the original error unchanged. Observer tests passed 5/5; observer plus accepted-response-loss transport tests passed 11/11; backend/script typechecks and sequence-6 offline-preflight passed with cloud/database writes 0. Instrumentation remains active because no reproduction was authorized.
24. **Completed 2026-07-29 — sequence-7 instrumented diagnostic successor:** exact read-only preflight proved branch `12/6`, sequence-6 parent revision/Run `1/1`, two unchanged terminal parent Attempts, empty sequence-7 prefix and open Cycle. The authorized guarded scope created WorkOrder `pi_experiment_revision_v2_t132_m7_l1_instrumented_diagnostic_successor_v7_1`, two cells, one admission and Run `ef_run_v2_t132_m7_l1_instrumented_diagnostic_successor_v7_1` / manifest `sha256:ad9196472551d493501884d02e6620d3ac5d7f680611a929c3e0c0eb069a56a1`. Normal T1-T4 added exactly 40 rows, relay delivered 3/3 and branch advanced to `14/7`; 236 protected tables changed 0. In-process and independent-process replay added 0 rows. Live runner rebind/typecheck and zero-cloud offline-preflight passed with Attempts/cloud/database writes 0.
25. **Completed 2026-07-29 — separately authorized sequence-7 read-only image preflight:** a fresh controller STS passed six exact keys, `0600`, temporary identity, exact role/policy and 59-minute validity checks. One `GetImage` matched the exact frozen Bundle and produced request hash `09e466d5af908f548b362c37753050921a9e12a9deade4c9ce5b6ed6acf64c50`; cloud calls 1, provider writes/`CreateJob`/database writes 0. Cloud Shell, `/tmp` and Downloads credential copies were removed.
26. **Completed but rejected 2026-07-29 — sequence-7 instrumented paid reproduction:** the exact two-Job/¥50 authorization and a fresh 59-minute controller STS opened the hard-fenced runner once. Both `CreateJob` calls produced top-level `400 / BadRequest` plus safe RequestIds; authenticated official diagnosis mapped both to `src property must be a valid json object`. Recovery found no Job/external ref. Attempts terminalized failed, commands terminalized after 12 passes, observed billable runtime was 0, scientific/evidence rows stayed 0 and all STS copies were removed. The observer proves safe error fields are top-level; the console-parity role-shape change is not sufficient.
27. **Completed offline 2026-07-30 — final SDK wire-boundary diagnosis:** after `APPROVE INSTRUMENTATION`, Debug Mode run `dbg-20260729-151747-2ddb` added a removable pre-`doAction` network blocker and structural observer. Focused tests passed 11/11 across the new observer, prior error observer and payload materializer; backend and script typechecks passed. Exact sequence-7 offline rematerialization produced final model/wire equality for both cells at 2,989/2,992 bytes with matching hashes, JSON round-trip/semantic equality, zero recursive `src`, four string-typed and object-parseable DataSource Options per request, absent `AssumeRoleFor`, cloud calls 0 and database writes 0. The SDK-rewrite hypothesis is ruled out; no Gate-2 behavior fix is yet supported.
28. **Completed offline/read-only 2026-07-30 — console-default-access isolation through sequence 8:** the console serializer comparison, network-blocked variant matrix and static RO/RW branch inspection isolated the untouched final RW mount as the one console-default differential. Gate 2 added a v2 request contract plus ordered RO-prefix/final-missing semantics while preserving exact v1 verification. Shared/backend/typecheck gates passed. Authorized named-local T1-T4 then created sequence 8 with exactly 40 rows, branch `16/8`, zero-new replay and no Attempt/evidence rows; a separately authorized fresh `GetImage` bound the official image to the frozen Bundle and left provider writes, `CreateJob`, database writes, Jobs and cost at 0.
29. **Completed but provider-rejected 2026-07-31 — sequence-8 paid verification:** after the earlier Cloud Shell/current-workstation stop, the original workstation passed exact offline and fresh `GetImage` gates under the renewed two-Job/¥50 authorization. The runner invoked `CreateJob` exactly twice; both returned HTTP 400 `BadRequest`, and authenticated diagnosis mapped both RequestIds to `src property must be a valid json object`. No Job/external ref or billable runtime resulted. Both Attempts and submit commands terminalized safely, scientific/evidence rows stayed zero, every credential copy was removed and the live authorization was disabled.
30. **Transferred 2026-07-30:** EF-P06 product candidate promotion, EF-P14 bound bootstrap/legacy-null handling, EF-P15 standalone attachment/revalidation and EF-P21 semantic retrieval are now owned by T-134. They no longer block T-132 and receive no implied implementation credit here.
31. **Submitted 2026-07-31 — provider escalation:** the owner explicitly authorized representational communication through the current Alibaba Cloud session. Ticket `000F4RKW18` was submitted at 22:05 Asia/Shanghai under `人工智能平台 PAI` with urgency `产品使用咨询`; the initial state is `已分派`, with the portal estimating an engineer update in 5-120 minutes. The ticket contains only the reviewed credential-free escalation text. T-132 remains `in-progress` and paused at the provider boundary; no sequence 9/10 or further `CreateJob` is permitted without a provider-supported correction and a new dated authorization.
32. **Completed 2026-08-01 — exact controller workspace permission and read proof:** support identified a pre-creation workspace-name lookup requiring `paiworkspace:GetWorkspace`. The owner authorized the exact workspace-scoped addition to `pea-m7-canary-controller`; repository/cloud policy v4 parity passed at SHA-256 `6566a47ee9c07ce6a75c9aeedcbc721d299ae52e7620bbbf91e14564b04220d8`. One separately authorized read returned `WorkspaceId=1450165 / Status=ENABLED`, RequestId `019FBC8E-4067-5E2E-9055-FD3C5D674F28`, with no Job or database write and complete STS cleanup.
33. **Completed 2026-08-01 — sequence-9 one-Job provider acceptance probe:** under the exact `2026-08-01 / maximum 1 Job / maximum ¥25` authorization, one and only one `CreateJob` call created `dlc1b1qk22drb4au`. It reached `Succeeded`; provider timestamps show creation `09:23:55Z`, running `09:26:05Z` and finish `09:26:13Z`. Exact recovery/collection returned result-manifest hash `sha256:40a5efb0cffc55ddd91dd0dfa495761508c1b3b2aeaec3b0dfe5e592665edd86`. No second `CreateJob` occurred; diagnostic database/scientific/evidence writes were 0; every temporary credential copy was removed. Observed runtime and selected CPU scale are safely below the ¥25 ceiling, while final provider billing remains the billing system's authority.
34. **Completed 2026-08-02 — durable two-cell closure:** the historical sequence-8 diagnostic path remains immutable. Its provider-supported correction was carried into the distinct durable sequence-9 successor, whose normal database-controlled flow completed both cells, collected both exact outputs and replayed with zero duplicate Jobs/Attempts/commands/outputs.
35. **Completed in source 2026-08-02 — durable successor preparation:** freeze database revision sequence 9 as `pi_experiment_revision_v2_t132_m7_l1_durable_two_cell_successor_v9_1` and Run `ef_run_v2_t132_m7_l1_durable_two_cell_successor_v9_1`, parented only by sequence 8 at branch state/head `16/8`. The successor preserves the two cells, frozen ExecutionBundle, `2 CPU / 8192 MiB`, one-attempt and 1800-second policy. Its max-40 named-local authorization remains `null`; no T1-T4 row has been written.
36. **Completed 2026-08-02 — named-local durable successor:** under the exact dated max-40/no-cloud authorization, normal T1-T4 materialized revision/Run sequence 9 with exactly 40 rows and branch `18/9`. Independent read-only census proved the exact 40-row family and prohibited rows 0; independent process replay returned all row deltas 0, 236 protected tables unchanged and prior lineage unchanged. WorkOrder/Run hashes are `sha256:b667db99ddcbe19267e604d4333d09f4d2cbd3635daab34b306453bf97b96482` / `sha256:c74bea341813166132f42b6398356a23aaf4785dfdb8e77a75efad5597473cea`. The Run hash is pinned, offline preflight passes with Attempts/cloud/database writes 0 and the named-local token is retired to `null`.
37. **Completed 2026-08-02 — sequence-9 read-only cloud gate:** one fresh 3,600-second controller STS used an inline session policy restricted to `paiimage:GetImage`. Its six-key local contract passed at mode `0600` with 57 whole minutes remaining. Exactly one production `GetImage` matched the pinned sequence-9 Run/Bundle and returned request hash `26ecc9878fc066d675ca011f83890884a05e3d4cf314eb2e66737e1c6932d08e`; provider writes, `CreateJob`, database/capability/scientific writes and paid runtime remained 0. Cloud Shell, `/tmp` and Downloads credential copies were removed.
38. **Completed 2026-08-02 — paid durable execution:** under the exact maximum-two-Job/¥50 authorization, the normal sequence-9 path submitted exactly two distinct Jobs and both reached `Succeeded`. Two Attempts terminalized `real_provider_succeeded`; six commands succeeded; two collection attempts each produced one exact `diagnostic_only` output. Replay added 0 rows/Jobs, protected tables were unchanged, scientific/capability writes were 0, credentials were removed and the dated source token was retired.
...
6. [x] Activate controller policy SHA-256 `c014cac58a794f2bc4849c0c05993ee85fc660dcb6d3206438b08bf7d5c219be`, obtain a fresh ≥55-minute STS and pass the production read-only `image-preflight`. The gate used one `GetImage` read and made zero provider writes, `CreateJob` calls or database writes.
7. [x] Enter the owner-confirmed paid window with a fresh controller STS. The first execute attempt passed exact offline lineage and one read-only `GetImage`, then failed closed before `CreateJob` on the v2 manifest persistence mismatch; Job, cost, Attempt and database-write counters stayed zero.
8. [x] Repair the normal Prisma write mapper and additive database CHECK for real-provider manifest v2 while keeping simulation v1-only; pass unit, relational, full disposable M7 and Prisma-drift gates with zero named-local/cloud writes.
9. [x] After separate approval, apply `20260728140500_enable_real_provider_payload_manifest_v2` to the reviewed named-local database and prove offline preflight plus zero-row/protected-table verification. Completed with 71/71 migrations, 250 protected application tables unchanged, exact Run Attempt count 0 and zero cloud calls.
10. [x] Execute the bounded sequence-8 paid verification once. On 2026-07-31 the exact two calls both returned HTTP 400 `BadRequest`; official diagnosis reports `src property must be a valid json object`. Recovery found no Job/external ref, both Attempts and commands terminalized safely, scientific/evidence rows stayed zero and all credentials were removed.
11. [x] Escalate the provider rejection with both RequestIds and the exact model/wire evidence. Completed through ticket `000F4RKW18`; provider identified the missing controller `paiworkspace:GetWorkspace` permission, the exact grant/read proof passed and the separately authorized one-Job diagnostic succeeded. The historical prohibition on sequence 9/10 is superseded only by that provider-supported correction; the consumed probe authorization remains unusable.
12. [x] Complete the personal-use landing through backend/API/CLI after provider acceptance: exact Run/Attempt/output visibility, zero-new replay, deterministic failure/reconcile/cancel coverage, credential/Job/database/cost cleanup and the operating/recovery runbook all passed. The final sequence-9 Jobs `dlcxhi03uhwc593e` / `dlcybgs96jh2qj75` succeeded with distinct exact diagnostic-only output hashes; desktop UI remains out of scope.
...
3. [ ] Run and collect one bounded two-cell real-provider diagnostic with both Jobs terminal-successful.
4. [ ] Close the backend/API/CLI review and replay loop; verify failure/reconcile/cancel with deterministic existing coverage without sacrificing a required successful live Job.
5. [ ] Record a repeatable personal operating procedure and recovery checklist.

Not in T-132: desktop UI, multi-user/tenant support, generalized BYOC packaging, installer/distribution and managed-cloud delivery. EF-P06/P14/P15 and semantic EF-P21 moved to T-134 and MUST NOT displace steps 3-5.

## 2026-07-30 — Console serializer lane recovery plan

1. [x] Confirm the console transport differential: control-plane requests are wrapped in POST `/data/api.json`, not sent directly to `/api/v1/jobs`.
2. [x] Stop active exposure and verify the unintended probe Job is terminal; record duration, displayed rate and boundary violation.
3. [x] Perform read-only inspection of the terminal Job configuration: confirm lifecycle, resource, image identity, one OSS mount, runtime role and synthetic command without changing cloud state.
4. [x] Inspect the static console serializer code and pin the exact `CreateJob` proxy route, encoding and outer field contract without using the Job form.
5. [x] Build and locally validate a temporary fail-closed proxy-body interceptor that blocks generic `/data/api.json` envelopes before transport and includes positive and false-negative cases for nested/stringified bodies.
6. [x] Under a dedicated Gate-1 approval, install a removable main-realm XHR guard, pass three synthetic block-before-send cycles and independently verify prototype cleanup without opening the Job form.
7. [x] Obtain a new explicit action-time authorization and perform the real-form structural capture. Run `dbg-20260730-071132-e186` blocked the exact `CreateJob` XHR before original `send`, retained only structure/count/hash evidence, restored all wrappers and proved the clone Job count remained zero.
8. [x] Compare the captured console-clone `content` schema offline against the exact sequence-7 SDK model and final wire body. Run `dbg-20260730-073606-f1c5` found 31 shared type paths, five console-only paths and nine sequence-7-only paths. It ruled the console defaults/clone metadata down to low-priority context and localized the unresolved differential to direct OSS `MountAccess`/mount cardinality, `Envs` and deterministic tags; no causal field or Gate-2 fix is supported.
9. [x] Under Gate-1 run `dbg-20260730-074202-25a0`, construct a network-blocked in-memory variant matrix over both exact sequence-7 cells. Baseline, omitted `MountAccess`, output-only and omitted `Envs` all passed SDK validation, final-wire equality, JSON roundtrip and zero recursive `src`; exact offline-preflight retained cloud/database writes `0/0`. The temporary matrix was removed and cleanup regression passed.
10. [x] Resolve whether the console serializer omits `MountAccess` universally or only for the default RW mount. Static run `dbg-20260730-075057-6d1c` proved that a new direct mount has no access value; the read-only switch writes `RO` when enabled and `RW` when explicitly disabled; the final serializer forwards that value. Thus an untouched RW mount omits the field, while a selected RO mount emits `RO`. No form projection was needed.
11. [x] Under Gate-1 run `dbg-20260730-080744-3c58`, exercise one removable exact sequence-7 in-memory `console-default-access` variant behind the pre-`doAction` network blocker. Both cells retained four sources and three RO values, omitted only the RW output access key, changed exactly one source, preserved all other semantics, and passed SDK/model/wire/roundtrip/zero-`src` checks at 2,970/2,973 bytes. Cloud/database writes remained `0/0`; current-run instrumentation was removed and cleanup regression passed.
12. [x] After `APPROVE FIX`, implement and verify the minimal no-cloud console-parity fix. The historical v1 request validator remains available for immutable payload verification; generation uses a v2 contract that disallows explicit RW. Ordered verification accepts only the historical RO-prefix/final-RW v1 shape or the new RO-prefix/final-missing v2 shape. Shared schema tests passed 6/6, the focused backend suite passed 12/12, all three typechecks passed and exact sequence-7 offline preflight retained model/wire equality with cloud/database writes `0/0`.
13. [x] Under the separately granted named-local authorization, add the fail-closed sequence-8 scope and admit the console-default-access successor. Read-only preflight proved branch `14/7`, exact sequence-7 parent and an empty sequence-8 prefix. T1-T4 added exactly 40 rows, relay delivered 3/3, branch advanced to `16/8`, 236 protected tables remained unchanged, and built-in plus independent-process replay added 0 rows. Independent census proved exact lineage 40 and prohibited rows 0.
14. [x] Under the separate sequence-8 read-only image-preflight authorization, acquire one fresh controller STS, pass the exact local credential gate, call one `GetImage` and bind the regional image to the frozen ExecutionBundle. Request hash `fbcba6059e58d7d7366f2872fbcaaf1e9297f5567087ccdeec157d4f7e95dcd1`; cloud calls 1; provider/`CreateJob`/database/capability/Job/cost effects 0. Cloud Shell, `/tmp` and Downloads copies were removed.
15. [x] Resume sequence-8 on the original workstation under the explicit 2026-07-31 two-Job/¥50 authorization. Exact offline and image preflights passed; the runner stopped at two rejected `CreateJob` calls, recovery found no Job and credential cleanup passed.
16. [x] Prepare the provider escalation package from the two RequestIds, exact wire hashes, request-shape census and zero-Job/zero-evidence database census at `artifacts/implementation/27-m7-l1-sequence8-provider-escalation.md`. No further live submission is authorized.
17. [ ] Submit the reviewed package through Alibaba Cloud support after explicit action-time confirmation. Do not attach credentials, full unredacted payloads or database dumps.

The safe console-clone structural comparison and exact local variant matrix are complete, but provider causality remains unresolved. The unintended synthetic Job is outside named-local lineage and cannot satisfy the M7 execution or scientific evidence gates.
