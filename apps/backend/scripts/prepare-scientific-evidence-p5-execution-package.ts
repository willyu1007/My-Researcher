...
const BRANCH_ID = 'pi_experiment_branch_v2_t136_p5_scifact_v1_1';
const REVISION_ID = 'pi_experiment_revision_v2_t136_p5_scifact_v1_1';
const RUN_ID = 'ef_run_v2_t136_p5_scifact_v1_1';
const PROTOCOL_REVISION_ID = 'ef_revision_t136_p5_protocol_scifact_micro_recall_v1';
const BUNDLE_REVISION_ID = 'ef_execution_bundle_revision_e87768c5205729b01ff8ceec8a8d0aaa69a15c3b';
const BUNDLE_REVISION_HASH = 'sha256:ea9cf75556dff3c34938c8937089b223dc8eb9513c658b722932e55bebce1437';
const P5_ATTEMPT_ID = 't136-p5-scifact-attempt-10';
...
const PREPARED_AUTHORIZATION_V12_PATH = path.resolve(
...
const PREPARED_AUTHORIZATION_V12_REF = 'manifests/prepared-authorization-v12.json';
...
const SESSION_POLICY: ScientificEvidenceP5ControlPlaneSessionPolicyV1 = {
  ...
  Statement: [
    ...
    },
    {
      ...
      Resource: 'acs:oss:*:*:pea-m7-canary-6194-202607/output/t136-p5/scifact/attempt-10/*',
    },
    {
...
async function main(): Promise<void> {
  ...
  try {
      ...
      provider: {
        ...
        profile: {
          ...
          workload_binding: {
            ...
            output_uri_prefix: 'oss://pea-m7-canary-6194-202607.oss-cn-shanghai-internal.aliyuncs.com/output/t136-p5/scifact/attempt-10/',
          },
        },
      },
    ...
    if (options.writeManifest) {
      await writeNewExactManifest(PREPARED_AUTHORIZATION_V12_PATH, serialized);
...
async function resolveSystemAssignedStart(writeManifest: boolean): Promise<string> {
  try {
    const existing = JSON.parse(
      await fs.readFile(PREPARED_AUTHORIZATION_V12_PATH, 'utf8'),
    ...
    if (!writeManifest) {
      throw new Error('T136_P5_REVISION_12_REQUIRES_FIRST_WRITE_MANIFEST');
    }
...
async function writeNewExactManifest(manifestPath: string, serialized: string): Promise<void> {
  try {
    ...
    assert.equal(existing, serialized, 'Existing revision-12 manifest differs from generated output.');
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error;
...
async function projectPreparedPackageToWorkloadProfile(packageHash: string): Promise<void> {
  ...
  assert.equal(authorization.current_revision, 12);
  ...
  if (authorization.authorization_acceptance_ref !== null) {
    ...
    assert.equal(authorization.prepared_authorization_ref, PREPARED_AUTHORIZATION_V12_REF);
    return;
  }
  ...
  assert.ok(
    ...
    'Workload profile already projects a different revision-12 package.',
  );
  assert.ok(
    authorization.prepared_authorization_ref === null
      || authorization.prepared_authorization_ref === PREPARED_AUTHORIZATION_V12_REF,
    'Workload profile already projects a different revision-12 manifest.',
  );
  ...
  const nextProfile: WorkloadProfile = {
    ...
    authorization: {
      ...
      prepared_authorization_ref: PREPARED_AUTHORIZATION_V12_REF,
