...
- The roadmap remains `in-progress`: M0-M4 technical/product-bound control work, M6 read-only cloud boundary and M7-I0..I3 default-off real-provider implementation are verified. Live real-provider execution/scientific activation, UI/search and release/non-local rollout remain incomplete.
- M7 implementation preserves the frozen new-lineage design: the existing immutable simulation Run cannot be dispatched as real; a new PI WorkOrder revision binds an exact typed ExecutionBundle and creates a new Run through T1-T4. T-132 owns M7; T-106 consumed final run `t132-m7-offline-20260723-v1` without adding provider authority. The next gate is separately authorized M7-L1 live diagnostic preparation/execution; any `CreateJob`, migration apply, scientific enable or non-local rollout remains independently fenced.
