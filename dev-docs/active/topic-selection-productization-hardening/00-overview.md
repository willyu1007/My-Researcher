...
- Next: **Phase 5 收尾**（M5 复杂度治理，用户裁定按推荐序全做）：① **5.2 压缩/token 规范化**（F-10：per-provider token 估计 + profile 驱动压缩策略 + STEP-7 debate 压缩 facts；非 D3，先做）→ ② **5.3 非 harness 优化**（coordinator 投影增量化 / loadTraces kind 过滤 / validateRegistry 缓存 / S5 并行化 / POLICY_VERSION·uniqueRefs 共享常量收编）→ ③ **5.1 harness 单文件拆分**（F-11，12,929 行，D3 敏感 + replay byte-identity 关键——先起草 D-T123-03 联合决策 + replay-identity 守卫，方案签核后再拆，于该签核点 checkpoint）。**遗留生成路径移除已于 Phase 1 完成**（三 service 已是纯读投影、零删除方法调用方；先前"~5.8k 行待移除"注记为陈旧，实已收口）。
- Task ID: `T-123`
