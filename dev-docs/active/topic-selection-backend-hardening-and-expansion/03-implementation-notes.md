...
- **子切片序列（data-up,各自独立可发,f0–f5 对运行中 harness 不可见,直到 f6 翻活）**:
  ...
  - **f3 = N6 admission-service**（types + `FORBIDDEN_AUTHORITY_KEYS` + N6 blocker 码 + admission 逻辑〔role order/arity、forbidden keys、prior-role hash chain、transcript `[slot,arity]` 重折比对〕,经 `ExpectedIdentityBuilder` 回调解耦 strategy）——依赖叶,可用 fake builder + 合成 artifact 独立单测。arity 源 = frozen scenario instance_policy（strategy 与 admission 共读,无环）。
