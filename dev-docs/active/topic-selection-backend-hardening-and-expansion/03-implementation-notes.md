...
- **子切片序列（data-up,各自独立可发,f0–f5 对运行中 harness 不可见,直到 f6 翻活）**:
  ...
  - **f5 = runtime entry** `V1bN6DivergentDebateRuntimeService.runDivergentDebate`（同文件:resolve handoff〔OQ4:倾向抽共享 resolveSharedN6RuntimeContext,镜像 N8〕 + 调 `core.runDivergentLoop` + arbiter 拆裸 5-key draft + gate bridge〔复用既有单 agent draft 路径,保 single-agent 身份〕;`loop_transcript_hash` 用 divergent `[slot,arity]` fold）。
