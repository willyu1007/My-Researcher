...
34. **Remaining — durable two-cell closure:** sequence 9 consumed its complete one-call allowance and intentionally bypassed durable Pack B writes. The commit checkpoint keeps the reviewed comparator normalization and zero-create recovery but removes the consumed probe mode and dated authorization string. Keep sequence-8 Attempts immutable. Create a new immutable successor only under a separately reviewed authorization, execute both cells through the normal database-controlled flow, collect both exact outputs and replay with zero duplicate Jobs/Attempts/commands/outputs.
...
38. **Authorized 2026-08-02 — paid durable execution:** the owner confirmed one exact sequence-9 window with at most two PAI Jobs and total ceiling ¥50. Action-time offline preflight passed with exact Run/manifest/Bundle and Attempt/cloud/database-write counts `0/0/0`. Obtain one fresh controller STS, execute the normal database-controlled two-cell intake once, require two terminal successes and exact diagnostic-only collection, replay with zero duplicate Job/Attempt/command/output, remove all credentials and retire the dated source token. Capability and scientific-evidence promotion remain prohibited.
...
12. [ ] Complete the personal-use landing through backend/API/CLI after provider acceptance: prove exact Run/Attempt/result visibility, zero-new replay, deterministic failure/reconcile/cancel coverage, credential/Job/database/cost cleanup and an operating/recovery runbook. Desktop UI is not required.
