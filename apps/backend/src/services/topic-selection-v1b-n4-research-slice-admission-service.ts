...
export type TopicSelectionV1bN4ResearchSliceSlotId = Extract<
  TopicSelectionV1bWorkflowHarnessSemanticSlotId,
  'n4_research_slice_option_draft'
>;

export type TopicSelectionV1bN4ResearchSliceAdmissionBlockerCode =
  | 'N4_DRAFT_ARTIFACT_PROVENANCE_MISSING'
  | 'N4_DRAFT_ARTIFACT_PROVENANCE_CLASS_INVALID'
  | 'N4_DRAFT_ARTIFACT_PROMPT_IDENTITY_DRIFT'
  | 'N4_DRAFT_ARTIFACT_PROFILE_DRIFT'
  | 'N4_DRAFT_ARTIFACT_RUNTIME_CONTEXT_DRIFT'
  | 'N4_DRAFT_ARTIFACT_SOURCE_HASH_DRIFT'
  | 'N4_DRAFT_ARTIFACT_PAYLOAD_HASH_MISMATCH'
  | 'N4_DRAFT_ARTIFACT_LEGACY_UNVERIFIED';

export interface TopicSelectionV1bN4ResearchSliceAdmissionIssue {
  code: TopicSelectionV1bN4ResearchSliceAdmissionBlockerCode;
  message: string;
}

export interface TopicSelectionV1bN4ResearchSliceAdmissionExpectedIdentity {
...
export interface TopicSelectionV1bN4ResearchSliceAdmissionInput {
  artifact: TopicSelectionV1bWorkflowHarnessSemanticSupportArtifactRef;
  expected: TopicSelectionV1bN4ResearchSliceAdmissionExpectedIdentity;
  allow_fixture_replay: boolean;
}

export type TopicSelectionV1bN4ResearchSliceAdmissionResult =
  | {
    admitted: true;
    artifact: TopicSelectionV1bWorkflowHarnessSemanticSupportArtifactRef;
  ...
  | {
    admitted: false;
    blocker: TopicSelectionV1bN4ResearchSliceAdmissionIssue;
  };
...
export class TopicSelectionV1bN4ResearchSliceAdmissionService {
  admit(
    input: TopicSelectionV1bN4ResearchSliceAdmissionInput,
  ): TopicSelectionV1bN4ResearchSliceAdmissionResult {
    ...
    if (
      ...
      return this.block(
        'N4_DRAFT_ARTIFACT_PROFILE_DRIFT',
        'v1b N4 research-slice draft artifact slot, effect, or output contract does not match the expected runtime slot.',
      );
    }
    ...
    if (
      ...
      return this.block(
        'N4_DRAFT_ARTIFACT_PAYLOAD_HASH_MISMATCH',
        'v1b N4 research-slice draft artifact payload hash does not match the normalized output hash.',
      );
    }
    ...
    if (!artifact.runtime_provenance_class) {
      return this.block(
        'N4_DRAFT_ARTIFACT_PROVENANCE_MISSING',
        'v1b N4 research-slice draft artifact is missing runtime provenance class.',
      );
    }
    ...
    if (artifact.runtime_provenance_class === 'legacy_unverified') {
      return this.block(
        'N4_DRAFT_ARTIFACT_LEGACY_UNVERIFIED',
        'legacy_unverified research-slice draft artifacts cannot enter promoted v1b N4 admission: the draft has no runtime-verified v1b provenance (it predates promotion or was produced outside the v1b runtime). Re-run N4 through the v1b harness so the draft is admitted as runtime_verified — a v1a or legacy artifact cannot be reused directly; v1a outputs must re-enter via v1b N1 (create-intake-snapshot).',
      );
    }
    ...
    if (artifact.runtime_provenance_class === 'fixture_replay') {
      if (!input.allow_fixture_replay) {
        return this.block(
          'N4_DRAFT_ARTIFACT_PROVENANCE_CLASS_INVALID',
          'fixture_replay N4 research-slice draft artifacts are allowed only in test or acceptance fixture mode.',
        );
      }
      return {
        admitted: true,
        ...
        warnings: ['fixture_replay_n4_research_slice_draft_admitted'],
      };
    }
    ...
    if (artifact.runtime_provenance_class !== 'runtime_verified') {
      return this.block(
        'N4_DRAFT_ARTIFACT_PROVENANCE_CLASS_INVALID',
        'v1b N4 research-slice draft artifact runtime provenance class is not recognized.',
      );
    }
    if (artifact.execution_mode !== 'codex_assisted' && artifact.execution_mode !== 'mocked_llm') {
      return this.block(
        'N4_DRAFT_ARTIFACT_PROVENANCE_CLASS_INVALID',
        'runtime_verified v1b N4 research-slice draft artifacts must be generated by the N4 research-slice runtime.',
      );
    }
    if (artifact.run_mode !== input.expected.run_mode) {
      return this.block(
        'N4_DRAFT_ARTIFACT_RUNTIME_CONTEXT_DRIFT',
        'v1b N4 research-slice draft artifact run_mode does not match the current runtime admission.',
      );
    }
    ...
    if (profileDrift) {
      return this.block(
        'N4_DRAFT_ARTIFACT_PROFILE_DRIFT',
        'v1b N4 research-slice draft artifact profile identity does not match the current runtime profile.',
      );
    }
    ...
    if (
      ...
      return this.block(
        'N4_DRAFT_ARTIFACT_PROMPT_IDENTITY_DRIFT',
        'v1b N4 research-slice draft artifact prompt identity does not match the current runtime prompt identity.',
      );
    }
    ...
    if (
      ...
      return this.block(
        'N4_DRAFT_ARTIFACT_RUNTIME_CONTEXT_DRIFT',
        'v1b N4 research-slice draft artifact provenance must point to the runtime audit artifact.',
      );
    }
    ...
    if (
      ...
      return this.block(
        'N4_DRAFT_ARTIFACT_RUNTIME_CONTEXT_DRIFT',
        'v1b N4 research-slice draft artifact runtime invocation or audit identity does not match current runtime context.',
      );
    }
    ...
    if (!this.sourceHashesMatch(artifact.source_hashes, input.expected.source_hashes)) {
      return this.block(
        'N4_DRAFT_ARTIFACT_SOURCE_HASH_DRIFT',
        'v1b N4 research-slice draft artifact source hashes do not match the current frozen planning lineage.',
      );
    }
    ...
    if (!this.compressionIdentityIsCoherent(artifact)) {
      return this.block(
        'N4_DRAFT_ARTIFACT_PROMPT_IDENTITY_DRIFT',
        'v1b N4 research-slice draft artifact compression identity is incomplete.',
      );
    }
