...
| Current v1c frontier | Machine Gate remains independently `ready_for_human_decision`, but the central checkpoint chain correctly overrides it and blocks promotion. The earliest repair point is a new candidate arena and strict-human gap confirmation against the refreshed EvidenceMap, followed by a fresh downstream question/package/v1c lineage and a newly snapshot-bound promotion confirmation. |

## Resume guard

The historical evidence decisions remain durable, both earlier candidate arenas are materialized, and the selected positive candidate remains a human-confirmed ValidatedNeed, but that gap authority is now truthfully upstream-stale relative to refreshed evidence checkpoint `research_checkpoint_19a6227d-3849-4d0e-ab6f-9e89e072b457`. Preserve the negative arena as a non-advancing control. The bounded two-paper acquisition, parsing, curation, and EvidenceMap refresh are complete, and downloader `max_byte_size` is restored to `1024`. Do not retry the old conditional promotion: the product has already materialized pending promotion checkpoint `research_checkpoint_35406533-fac3-4cdb-b6d9-29296ca66c63` and returned `CHECKPOINT_CHAIN_INCOMPLETE`. Resume at the earliest unsatisfied checkpoint by generating current-evidence NeedCandidates and a gap arena; stop for exact strict-human gap confirmation, then rebuild every downstream checkpoint so each carries the current predecessor ref. Do not invoke a provider, record a gap or promotion Human authority, or create a bridge without the corresponding new authorization boundary. The configured raw-file root is transient; if its OA PDFs disappear, obtain exact authorization and reacquire only those two assets through a new bounded dry-run rather than overwriting storage settings.
