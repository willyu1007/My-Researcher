...
- **路由注册**：`app.ts` 统一解析 title-card management store config；当 `TITLE_CARD_REPOSITORY=prisma` 时，会把 research-lifecycle / literature / auto-pull 同步级联到 Prisma，并对 mixed-store 配置 fail-fast。service 额外注入 `TopicManagementReferenceGateway`，从 auto-pull repo 校验 topic profile、从 literature repo 校验 literature existence。
