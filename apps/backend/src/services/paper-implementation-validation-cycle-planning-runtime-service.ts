...
import {
  ...
  PAPER_IMPLEMENTATION_VALIDATION_CYCLE_PLANNING_ROLE_OUTPUT_SCHEMA_ID,
...
export interface PaperImplementationValidationCyclePlanningRuntimeResult {
  run_id: string;
...
export type PaperImplementationValidationCyclePlanningAgentOrchestrator =
...
interface RuntimeServiceOptions {
  ...
  agentOrchestrator: PaperImplementationValidationCyclePlanningAgentOrchestrator;
...
interface SlotProfile {
  slotId: PaperImplementationValidationCyclePlanningSlotId;
  workflowType: 'validation_cycle_planning';
  profileId: string;
  promptTemplateId: string;
  promptTemplateVersion: string;
  contextPolicyId: string;
  roleSlotId: PaperImplementationValidationCyclePlanningRoleSlotId;
...
interface RoleInvocationOutcome {
  result: TopicSelectionAgentInvocationResult<PaperImplementationValidationCyclePlanningRoleOutput>;
...
const CYCLE_CANDIDATES_PROFILE: SlotProfile = {
  slotId: PAPER_IMPLEMENTATION_VALIDATION_CYCLE_PLANNING_SLOT_ID,
  workflowType: 'validation_cycle_planning',
  profileId: PAPER_IMPLEMENTATION_VALIDATION_CYCLE_PLANNING_PROFILE_ID,
  promptTemplateId: PAPER_IMPLEMENTATION_VALIDATION_CYCLE_PLANNING_PROMPT_TEMPLATE_ID,
  promptTemplateVersion: PAPER_IMPLEMENTATION_VALIDATION_CYCLE_PLANNING_PROMPT_TEMPLATE_VERSION,
  contextPolicyId: 'paper-implementation.validation-cycle-planning.context-policy.v1',
  roleSlotId: PAPER_IMPLEMENTATION_VALIDATION_CYCLE_PLANNING_ROLE_SLOT_ID,
  roleExecutorKind: 'single_agent',
  promptVariantId: 'cycle-candidates.main',
  finalArtifactRefType: 'validation_cycle_planning_runtime_artifact',
  roleArtifactRefType: 'validation_cycle_planning_role_artifact',
  artifactContractId: 'ValidationCyclePlanningCycleCandidatesArtifact',
  promptPolicyId: 'paper-implementation.validation-cycle-planning.prompt-redaction.v1',
};
...
const RETRYABLE_RUNTIME_FAILURE_CODES = new Set<string>([
  ...
  'VALIDATION_CYCLE_PLANNING_CANDIDATE_KEY_MISMATCH',
  'VALIDATION_CYCLE_PLANNING_CANDIDATE_SET_INCOMPLETE',
  'VALIDATION_CYCLE_PLANNING_SIDE_EFFECT_GUARD_MISSING',
]);

export class PaperImplementationValidationCyclePlanningRuntimeService {
  ...
  private readonly agentOrchestrator: PaperImplementationValidationCyclePlanningAgentOrchestrator;
  ...
  private async invokeRoleWithBoundedRetry(
    ...
    throw new AppError(500, 'INTERNAL_ERROR', 'Validation cycle planning runtime retry loop exhausted unexpectedly.');
  }
  ...
  private async invokeRole(
    ...
    request: RunPaperImplementationValidationCyclePlanningRuntimeRequest,
    retryAttemptIndex: number,
  ): Promise<TopicSelectionAgentInvocationResult<PaperImplementationValidationCyclePlanningRoleOutput>> {
    ...
    return this.agentOrchestrator.invokeStructuredOutput<PaperImplementationValidationCyclePlanningRoleOutput>({
      ...
      output_contract: PAPER_IMPLEMENTATION_VALIDATION_CYCLE_PLANNING_ROLE_OUTPUT_SCHEMA_ID,
  ...
  private async recordPreflightBlockedArtifact(
    ...
    const output: PaperImplementationValidationCyclePlanningRoleOutput = {
      role_slot_id: runtimeBase.profile.roleSlotId,
      role_status: 'blocked',
      summary: 'Deterministic validation-cycle planning preflight found blockers before semantic planning.',
      ...
      reviewed_candidate_keys: this.uniqueStrings(request.reviewed_candidate_keys),
    ...
    const artifact = this.buildRuntimeArtifact(runtimeBase, request, {
      ...
      outputSchemaId: PAPER_IMPLEMENTATION_VALIDATION_CYCLE_PLANNING_ROLE_OUTPUT_SCHEMA_ID,
  ...
  private async recordFinalArtifact(
    ...
    const finalArtifact = this.buildRuntimeArtifact(runtimeBase, request, {
      ...
      artifactContractVersion: 'v1',
      outputSchemaId: PAPER_IMPLEMENTATION_VALIDATION_CYCLE_PLANNING_FINAL_OUTPUT_SCHEMA_ID,
  ...
  private finalPayload(
    ...
    return {
      ...
      reviewed_route_skeptic_artifact_hash: request.admitted_route_skeptic_artifact_hash,
      reviewed_candidate_keys: this.uniqueStrings(request.reviewed_candidate_keys),
      cycle_candidate_proposals: roleOutput?.cycle_candidate_proposals ?? [],
      no_domain_gate_request: true,
      no_queue_side_effect: true,
      no_validation_cycle_side_effect: true,
  ...
  private contextPolicyProfile(profile: SlotProfile): TopicSelectionContextPolicyProfile {
    return {
      ...
      context_family: 'paper_implementation_validation_cycle_planning',
  ...
  private assertRequest(request: RunPaperImplementationValidationCyclePlanningRuntimeRequest): void {
    ...
    if (requestedProfileId && requestedProfileId !== CYCLE_CANDIDATES_PROFILE.profileId) {
      throw new AppError(
        400,
        'INVALID_PAYLOAD',
        `model_profile_id must match runtime slot profile ${CYCLE_CANDIDATES_PROFILE.profileId}.`,
      );
    }
    ...
    if (
      ...
      && !requestedModelOptionId.startsWith(`${CYCLE_CANDIDATES_PROFILE.profileId}.`)
    ) {
      throw new AppError(
        400,
        'INVALID_PAYLOAD',
        `model_option_id must belong to runtime slot profile ${CYCLE_CANDIDATES_PROFILE.profileId}.`,
      );
    }
    if (request.execution_mode === 'mocked_llm' && !request.mocked_role_outputs?.[CYCLE_CANDIDATES_PROFILE.roleSlotId]) {
      throw new AppError(400, 'INVALID_PAYLOAD', `mocked_role_outputs.${CYCLE_CANDIDATES_PROFILE.roleSlotId} is required.`);
    }
    if (request.execution_mode === 'codex_assisted' && !request.codex_role_outputs?.[CYCLE_CANDIDATES_PROFILE.roleSlotId]) {
      throw new AppError(400, 'INVALID_PAYLOAD', `codex_role_outputs.${CYCLE_CANDIDATES_PROFILE.roleSlotId} is required.`);
    }
    if (!request.admitted_route_proposal_artifact_ref || !request.admitted_route_proposal_artifact_hash) {
      throw new AppError(
        400,
        'INVALID_PAYLOAD',
        'validation_cycle_planning requires admitted_route_proposal_artifact_ref and admitted_route_proposal_artifact_hash.',
      );
    }
    if (!request.admitted_route_skeptic_artifact_ref || !request.admitted_route_skeptic_artifact_hash) {
      throw new AppError(
        400,
        'INVALID_PAYLOAD',
        'validation_cycle_planning requires admitted_route_skeptic_artifact_ref and admitted_route_skeptic_artifact_hash.',
      );
    }
    if (!request.reviewed_candidate_keys || request.reviewed_candidate_keys.length === 0) {
      throw new AppError(
        400,
        'INVALID_PAYLOAD',
        'validation_cycle_planning requires at least one reviewed_candidate_keys entry.',
      );
    }
  }

  private hasRoleOutputs(
    outputs: RunPaperImplementationValidationCyclePlanningRuntimeRequest['mocked_role_outputs']
      | RunPaperImplementationValidationCyclePlanningRuntimeRequest['codex_role_outputs'],
  ): boolean {
  ...
  ): PaperImplementationValidationCyclePlanningRuntimeResult {
    return {
      run_id: runtimeBase.runId,
      slot_id: runtimeBase.profile.slotId,
      workflow_type: runtimeBase.profile.workflowType,
  ...
  private semanticOutputFailureCode(
    ...
    if (
      ...
      return 'VALIDATION_CYCLE_PLANNING_PRIMARY_INPUT_MISSING';
    }
    if (
    ...
    if (!output.cycle_candidate_proposals || output.cycle_candidate_proposals.length < 2) {
      return 'VALIDATION_CYCLE_PLANNING_CANDIDATE_SET_INCOMPLETE';
    }
    if (
      ...
      return 'VALIDATION_CYCLE_PLANNING_SIDE_EFFECT_GUARD_MISSING';
    }
    return null;
  }
  ...
  private candidateProposalsMatchReviewedKeys(
    ...
    return (output.cycle_candidate_proposals ?? []).every(
  ...
  private titleCardId(request: RunPaperImplementationValidationCyclePlanningRuntimeRequest): string | null {
