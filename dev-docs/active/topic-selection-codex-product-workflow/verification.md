...
| Real model qualification / CX-02, CX-08 | Pinned abstract cases through attempt 75 | N6 insufficiency/apparent conflict and exact-delta positive/negative review inspected; ordinary N8 v4 admitted | 1,896,976 reported tokens plus unknown usage from five failed attempts; conditional N8 v5 admitted; regeneration and shipped activation remain. All profiles closed. See current evidence below. |
...
`/tmp/my-researcher-t153-phase2/live/`. Sources and setup boundaries are defined above. The accounting snapshot through attempt 75 is **1,896,976 reported tokens and five unknown-usage attempts (4, 8, 9, 20, 63)**.
...
time are null under the user's amendment; the original product timeout is 600 seconds. N6 regeneration qualification is ongoing in the same ledger; no budget approval is
pending. Every retry gets a new identity and files.
...
| N8 role-slot declarations and CLI operating note lagged integrated consumers | Four bounded-debate slots now declare CLI consistently with the existing runtime; the matrix follows those slots. The execution note now separates model-result reuse from domain replay, documents native tool policy, and distinguishes deferred gateway activation. Matrix check, 34 shared schema tests and backend typecheck passed | All shipped profiles remain closed; slot alignment alone does not activate them. |
...
- Complete actual N6 regeneration and final-gate overclaim refusal using
  fresh attempt identities; inspect all semantic results, not just the opt-in test's exit code.
- Open only the proved profiles together with scenario/policy/operating-doc alignment, then run
  without a qualification-local registry override. Every shipped profile remains closed today.
...
- Phase 2 is not closed. Phase 3 is authorized afterward: sampling/extraction, need discovery/final
  synthesis/adjudication/confirmation support, evidence convergence, then qualification and a fresh
  upstream-to-v1b lineage. Phases 4–5 remain required outside current authorization.
- Whole-flow CX-06/CX-08 must distinguish actual models, controlled fixtures and genuine Human
  decisions. T-129 archival separately awaits reconciliation of its second worktree occurrence.
