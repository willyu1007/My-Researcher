...
- **W-13 v1c-N2 + v1c-N4 生产接线/收口**（审计 `wf_034f15eb` 确认二者皆 dead，仅 canary inline 自证）：① **v1c-N2** bounded debate——拆 `ROLE_OUTPUT_SCHEMA` 为 per-slot const/enum + prompt 指令使 emission↔admission 对齐；接真生产 caller（捕获真 provider per-slot 响应）+ 接 `createPromotionDecisionSupportFromVerifiedRuntimeDraft` gate 入口；修不一致 fixtures。② **v1c-N4** delegated-promotion——`recordHumanPromotionDecision` 穿 delegated candidate + `app.ts` 构造 runtime/admission，或**显式文档化为 reserved/dormant**（产品决策：是否需要 delegated promotion 决定路径）。**两者各需先定「接真 caller vs reserved」**；触碰 harness/orchestrator 走 T-088 `D-T128-0N` JD。
