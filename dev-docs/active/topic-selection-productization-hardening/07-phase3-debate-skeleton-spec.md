...
**Increment B — full-loop e2e (TODO, the remaining DP-3.6 deliverable):** drive N1→N7→N8 with a gate-valid BORDERLINE draft (total_score∈[60,72) → T1) → assert first-pass loopback + `N8ToN7Feedback` persisted → N7 feedback re-entry admits (the coordinator-assembled feedback input must pass the REAL harness parser — the integration confidence the unit test's StubHarness can't give) → caller-side `runDebate` (4 mocked roles) → N8 re-eval admits with `N8_VALUE_BORDERLINE_AFTER_DEBATE` warning (firstPass=false, no re-loop). Home: `topic-selection-v1b-routes.integration.test.ts` (has the N8 helpers + drives the harness HTTP).
