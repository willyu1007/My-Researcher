...
Phase 1 realized these shapes through the smallest existing owners: `LiteratureResourcePoolSnapshot` is the managed-library corpus manifest and stores sorted embedding-version membership plus retrieval-stack identity; the search-recheck lifecycle owns normalized `request_key`/`strategy_key`, issue/session intent, corpus identity, execution policy, and resulting SearchPlan/SearchRun refs; immutable control-plane artifacts carry `EvidenceDelta` and `ResolutionRoute`; the closed round-link contract binds a successor Arena session to prior transcript and EvidenceDelta hashes. Only EvidenceMap successor/head state required a new persisted transition and compare-and-swap repository method. No parallel corpus, request, round, delta, or route store was introduced.

Canonical request identity normalizes whitespace and case in semantic text, sorts and deduplicates candidate queries, and normalizes absent versus explicit-null functional-ref version fields. Roles cannot supply `request_key` or `strategy_key`; the coordinator hashes the normalized strategy and request payloads. Equivalent concurrent Prisma writes converge on the existing request through the unique request key.
