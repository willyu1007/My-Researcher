...
5. **Local workload manifest completed 2026-07-27:** `workloads/ragperf-canary/manifests/workload-directory-v1.json` freezes the exact expanded single-file directory: `entrypoint.py`, SHA-256 `9b2a82298dfa969146e5e223893d3d86c6254cb16a995be72b65709a55b4f05d`, 7,916 bytes, and the exact future internal OSS directory ref. Its state is `not_uploaded`. The next D gate is an explicit confirmation for the external SciFact source download; OSS writes then require a second, separate confirmation before exact objects are uploaded and verified without enabling `CreateJob`.
