...
| Codex activation evidence | Keep C-2's same-corpus dependency versus qualify each enabled role using actual inputs and failure cases | Replace the inherited blanket corpus dependency for Codex with the qualification criteria below; retain optional calibration separately | decided | User approval, 2026-09-09 | Historical D-30 already removed C-1 sign-off from C-3; C-2's corpus coupling is superseded for Codex by the approved role-specific qualification | No constant-only flip, fabricated corpus or broad scientific-quality claim. |
...
|---|---|---|
| Existing runner outcomes are enough for sequential model roles and product-owned round carry-over. | Consumer work may require a coordinated interface change. | T-152 transport plus actual Phase 2–4 consumer runs verify fresh attempts, role carry-over and shutdown; Phase 5 composes the public workflow. |
| Canonical APIs can compose the complete selection flow with bounded additions. | “Usable” could remain a set of internal services with no operating entry point. | Phase 5 HTTP composition passes through managed literature, v1a, v1b, promotion, bridge/intake and feedback with the existing owners and explicit Human stops. |
| Existing artifact stable-key uniqueness can exclude duplicate metered attempts without a new persistence model. | A naïve idempotent create can let two callers both believe they own execution. | Concurrent relational attempt checks and Phase 5 PostgreSQL reconstruction verify exclusion and exact replay; ambiguous incomplete writes stay fail-closed. |
...
| C-2 six gated prompts | Completed for Codex under the replacement qualification approved 2026-09-09 | All 36 profiles qualified in Phases 2–4, including the historical six (ordinary N8 and promotion N2 included) and later refinement. Original C-2 corpus completion is not retroactively claimed. |
| C-3 live role outputs and bridge provenance | Completed for `codex_cli` across the complete workflow | Real role artifacts and Phase 5 HTTP composition retain CLI provenance through deterministic admission, promotion, bridge/intake and feedback. |
| C-3 run mode and execution-spec handling | Completed for canonical Codex consumers | Shipped defaults, request shape/admission checks and consumed execution specifications agree; no caller role answers or canary eligibility overrides in the composed path. |
...
| Old `calibration_gate_release` references | Reconciled | Runtime refusals, scenarios and operator guidance distinguish qualified product Codex from the unchanged deferred gateway gate. No blanket corpus or activation claim remains on the Codex path. |
...
3. Open only qualified profiles with permanent policy/scenario/operator documentation, then verify canonical progression, Human stops and recovery. Keep ambiguous partial domain commits fail-closed pending authority inspection. Subsequent authorization covers all five phases.

### Phase 3 — Connect upstream evidence and need discovery (complete)
- Current implementation: Sampling has stable submission recovery and real qualification. Original-abstract/bound-paragraph extraction, single-agent and complete Debate need discovery, adjudication, exact Human-confirmation support, three-role evidence convergence and optional Arena execute through their canonical Codex product callers. Complete role references are checked before downstream use; evidence remains intact through context compression. Fresh original-source extraction now composes actual evidence/gap checkpoint owners, two substantive candidate bodies, N7, fixed Human/N8 and a unique frozen v1b input. Completed and non-advancing results replay exactly; ambiguous partial domain writes remain fail-closed.
...
- Revertible implementation units after Phase 2 closeout:
  ...
  2. Need discovery and decision support: integrate single-agent generation, Explorer/DeepCritic/framing/final synthesis, adjudication and confirmation support. Single-agent and Debate need-discovery compile source/strength/conflict bodies and carry actual prior-role outputs through the final CLI slot. Original-abstract and bound-paragraph non-advance cases retain missing evidence and matching default profile hashes; adjudication and exact Human-confirmation support pass bounded/overclaim and positive/incomplete qualification. Frozen source bodies, exact Human input, complete references and JSON receipt recovery are verified.
...
- Closeout: CX-03 holds. Independent review verifies repaired five-role Debate non-advance/replay (169–174) and the checkpoint-bound original-source single-agent chain (175–178), including actual candidate comparison, all five residual risks, exact Human snapshot/input and unique frozen lineage. Together with sampling, convergence and Arena qualifications, no Phase 3 work remains. Human decisions and retrieval/readiness setup are controlled fixtures; this does not establish actual researcher approval, statistical scientific quality or full CLI relational recovery. The user subsequently authorized Phase 4 on 2026-09-10.
...
- Closeout: CX-04/CX-05 hold. N4 and all six optional v1b support profiles are implemented and qualified. Ordinary promotion support (207), four-role risk Debate (208–211), delegated candidate/Human acceptance (216) and feedback recheck/no-recheck (213/215) pass real Codex qualification with full original scientific context and exact recovery. All 36 topic-selection profiles admit product CLI by default; no Phase 4 role depends on external role answers or another generation provider. Model candidates remain separate from Human authority, and feedback only records its deterministic recheck outcome. Phase 5 subsequently verifies whole-flow composition and persistent recovery.
...
- Closeout: CX-06/CX-07/CX-08/CX-09 hold. The advancing canonical HTTP chain executes 16 CLI roles with no caller-authored answers or gateway calls, preserves explicit Human decisions, creates one PaperProject and records feedback. The same test passes with PostgreSQL repositories across application reconstruction and exact upstream/downstream replay. Review fixes managed-fulltext loading, ordinary N8 context budget with historical completed-gate compatibility, N4 audit duplication and intake partial-create recovery. Role qualifications remain valid; no new live model call or actual researcher approval is claimed. See verification.md for checks and limits.
