...
38. **Next authorization boundary — paid durable execution:** obtain a new explicit dated maximum-two-Job and monetary ceiling plus a fresh controller STS, then run the normal sequence-9 database-controlled intake, terminal synchronization, exact two-cell collection and zero-duplicate replay. The consumed read-only authorization does not permit `CreateJob`.
