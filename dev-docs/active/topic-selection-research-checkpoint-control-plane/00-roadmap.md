...
- When a current support-only arena exists, project its exact session, synthesis ref/hash, candidate dispositions, findings, and reopening conditions into the gap-checkpoint/HumanConfirmNeed packet and maintained human/LLM stage views. Arena absence does not block the existing human path during Phase 9.
- Require the strict-human gap review to record `accept`, `override`, or `defer` for advice that is actually exposed, bound to that exact arena snapshot. Acceptance or override may inform the existing human decision; the arena itself cannot select, drop, advance, or block a candidate during Phase 9.
- Record reason-code-bucketed override/reopen labels for Phase 10 calibration. Reuse the existing continuation envelope unchanged except for refreshed manifest/artifact refs; routine local execution still stops only at the next semantic decision or another existing effect boundary.
- Exit when low-value/no-topic advice is legible, an advancing human decision remains bound to current product authority, stale arena advice cannot be reviewed, override labels are recoverable, and no extra node-level authorization is introduced.
- Verification: Advisory accept/override/defer cases, stale/superseded arena rejection, no-topic/park/select packet rendering, artifact regeneration, continuation-envelope regression, OpenAPI/context alignment, and full relevant typechecks.
...
- Dependencies: Phase 9 advisory integration and researcher-approved dominance pairs, perturbation cases, and activation thresholds. Phase 9 override labels supply evidence; they do not authorize activation.
...
- Status: ready
- Authorized boundary: through Phase 9B (complete); Phase 9C requires separate review and authorization
