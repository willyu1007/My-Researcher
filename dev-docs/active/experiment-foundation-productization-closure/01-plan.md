...
36. **Authorized 2026-08-02 — named-local only:** the owner confirmed one dated max-40/no-cloud authorization for the prepared sequence-9 scope. Run normal T1-T4 and exact replay, require branch `18/9`, 40 new rows, protected tables unchanged and prohibited effects 0. Pin the returned server-owned Run manifest hash into the normal live scope, then run zero-cloud offline gates. This authorization does not include STS, cloud reads, provider writes, `CreateJob` or paid execution.
