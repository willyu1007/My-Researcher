...
- `workloads/ragperf-canary/manifests/scifact-authority-v1.json` is the reviewed authoring input. The manifest pins the exact BEIR archive MD5/SHA-256, upstream SciFact license revision, two policy snapshots, two dataset source/split/checksum snapshots, deterministic server revision IDs and idempotency keys. The manifest contains no credential, cloud capability or live execution grant.
...
- Named-local apply is a distinct authorization boundary and uses the same service/repository semantics with the reviewed target fingerprint, exact replay and protected-table negative-space checks. Its persistence census is table-level, not operation-level: 10 append-only lifecycle events necessarily maintain four current-state `ExperimentFoundationAssetLifecycleProjectionV2` rows, so the complete bound is 26 rows rather than 22. The importer requires an exact 26-row process authorization, rejects the obsolete 22-row string before database connection, and treats every projection as rebuildable state sourced from its exact latest event—not as a second semantic authority.
- Protected-table negative space fingerprints every non-target application row by ordered primary-key values plus PostgreSQL `xmin`. Inserts/deletes change membership; any update changes the row version. The row-version signature avoids reading large vector/payload columns while retaining mutation detection across all protected tables. The completed apply/replay makes the two Dataset mirror bindings canonical; `ExecutionBundle@v2` remains a separate write/authorization boundary.
...
- The mounted workload directory and dataset objects use content-addressed object names and retain content digest + byte size in the reviewed ExecutionBundle/mirror manifests. The provider request must bind those exact refs; a mutable prefix, console display label or unbound local file is not executable authority.
...
- Implemented 2026-07-26: `AliyunPaiDlcRealProviderProfile@v1` adds one closed `AliyunPaiDlcWorkloadBinding@v1`. Materialization requires content-addressed internal OSS directory URIs in the exact regional bucket, distinct non-nested canonical mount roots, ordered mirror ordinals and a command rooted at the code mount. The materializer emits RO code/input mounts, one exact `output/<run_id>/<cell_key>/` RW mount, standard `EXPERIMENT_FOUNDATION_*` variables and the exact service-role credential structure.
- The transient SDK wire request is validated against a closed `AliyunPaiDlcCreateJobPayload@v1` schema before canonical hashing. Durable state never stores workspace/resource/image/command/URI/mount/env/credential/tag values; durable state stores their hashes or exact non-secret digest/byte-size evidence. Rematerialization binds role, mounts, environment and artifact sizes and fails closed on drift.
- The workload prefix mounted at the code path must contain `entrypoint.py` at its root; uploading only a compressed archive would not satisfy the frozen command path. The two ordered dataset mirrors expose `corpus.jsonl` and `queries.jsonl` at input ordinals 1 and 2.
- Direct DLC OSS mounting has its own PAI/DLC storage-authorization prerequisite. On 2026-07-27 the account-level “全部云产品依赖” page verified DLC → OSS data storage as `已开通`; the create-job form also exposed the OSS URI/mount-path/runtime-role controls without an authorization prompt. The console evidence verifies the platform dependency only. In-container runtime identity and exact prefix access still require live-window verification.
- Read-only `GetImage` verification resolved PAI asset `image-liuxvj7p2qcnflha84` to the exact regional `ImageUri` `dsw-registry-vpc.cn-shanghai.cr.aliyuncs.com/pai/torcheasyrec:1.3.0-pytorch2.12.1-cpu-py311-ubuntu22.04`. The API returned null `Identity` and `Signature` and no OCI/content digest. `ExecutionBundle@v1` therefore remains unchanged as the OCI-digest contract. `ExecutionBundle@v2` adds a separate `provider_managed_asset` identity that freezes provider, ImageId, region, modification time, byte size, accessibility and source type, and hard-limits use to `m7_l1_diagnostic_only`. Redacted manifest v2 stores only the hash of that typed identity. Neither ImageId nor a metadata hash is written into `image_digest`, and M7-L2 scientific evidence continues to require a content-digest image route.
...
## M7 implemented default-off provider boundary

M7 retains one authority chain. A WorkOrder v2 carries only an exact EF-owned ExecutionBundle revision ref/hash; EF resolves active readiness and derives the executable recipe/task snapshots. The ExecutionBundle content itself remains EF authority and is never caller-hashed or copied into PI mutable state.
...
## D-25 — Public-resource execution must bind an exact ECS spec

The M7-L1 live runner may not translate an arbitrary TaskSpec CPU/memory pair into a public-resource job. Public DLC jobs use a provider ECS specification. The real-provider profile therefore carries one exact `ecs_spec` plus its reviewed CPU/memory facts; the payload emits `EcsSpec` for `public_resource`, while `exact_quota` retains `ResourceConfig`. Profile materialization rejects any TaskSpec/profile resource mismatch.
...
## D-37 — Sequence-8 rejects console-default access as the sole provider cause
...
## D-40 — Durable revision sequence 9 is distinct from the consumed diagnostic window

The 2026-08-01 “sequence-9 probe” named an authorization window, not a persisted PI WorkOrder revision. The next database successor therefore uses the natural immutable revision sequence 9 while carrying explicit `durable_two_cell_successor_v9` identity. Its parent is exact sequence 8 at branch state/head `16/8`; it preserves the same two cells, ExecutionBundle, resource snapshot and one-attempt/1800-second policy. It does not attach the diagnostic Job or mutate either historical sequence-8 Attempt.

Preparation and materialization remain separate authority gates. Source may freeze the successor ids, parent CAS values, max-40 row census and intended title/objective while the apply authorization value is `null`. Normal T1-T4 materialization is the only authority for the new WorkOrder revision hash and Run manifest hash. The live runner must not guess either value: normal offline/image/execute modes target the new Run but fail before database access while its manifest hash is unpinned. Historical `sequence9-recover` retains a separate exact sequence-8 scope and zero-`CreateJob` ceiling.

After a separately approved named-local-only apply, the returned manifest hash is reviewed and pinned in source. Only then may zero-cloud payload/wire/replay gates run. Read-only image preflight and paid two-Job execution remain later, independent authorizations; both real-provider capabilities and the source-level paid token stay disabled throughout successor preparation.
