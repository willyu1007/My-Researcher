...
**STEP 11 regression — DONE (all green):** v1c-n2 byte-identity smoke exit 0, n8 runtime smoke exit 0, full N1→N11 v1b harness e2e exit 0, backend unit suite 1290 pass / 0 fail, tsc 0. (The only existing-file change in this slice is additive registry entries — validated by the green prompt-template lint gate, which failed until the entry was added.)

## 3.5 — coordinator feedback recipe (DP-3.6)

**Architecture confirmed against the locked decisions (03-notes line 66):** the debate runtime is a **caller-side service** (isomorphic with v1c N2) — the harness only does the deterministic gate (trigger detection + first-pass loopback); it does NOT invoke `runDebate` (DMP-10 / D-07 call boundary). The understand pass also confirmed ~80% of the contract layer is already built: the N7 `feedback_from_n8` parser (`resolveN7FeedbackPayload`), the N7 runner weaving `n8_debate_admission_ref` into the handoff + emitting `n7_n8_debate_admission_review` + the `n8_debate_level_selected` warning, the `N8ToN7Feedback@v1` contract, and `RB_N8_N7`. The ONE real gap was the coordinator dead-end: it could only re-invoke the loopback *source* (N8), not route the loopback *upstream* to N7 in feedback mode.

**Increment A — coordinator feedback re-entry — DONE + verified:**
- `HANDOFF_BUILDER_TABLE` N7 entry gains a `feedback_reentry` descriptor (loopback source = N8 + the input_mode/feedback-key names).
- `buildNextRequest` calls `resolveFeedbackReentry`: when an UNCONSUMED N8 loopback exists (its attempt seq is newer than N7's last admission), it overrides `input_mode='feedback_from_n8'` and threads `n8_feedback_ref` (= the loopback's authority_ref), `n8_feedback_hash` (= `canonicalHash` of the immutable feedback artifact record, reproduced), `n8_feedback_payload_hash` (= the loopback's authority_hash) into the frozen payload + adds the feedback ref to source_refs. Key-set correctness is by construction: the initial N7 build already matches the parser's base keys, so base + the 3 feedback keys = the parser's feedback key set exactly.
- `pendingHalt` exposes `loopback_target_node_id` (only when the target has a feedback recipe for that source); the retry logic allows `retry_node_id` = that upstream target (in addition to the existing source re-invoke); dead-end message replaced with the re-entry guidance.
- Tests: 2 new coordinator unit tests (feedback re-entry assembly byte-correct + source re-invoke still works) — coordinator 14/14, tsc 0, full backend suite 1292 pass / 0 fail.

**Increment B — full-loop e2e (TODO) + a NEWLY-FOUND coordinator gap.** A draft coordinator-driven e2e (`topic-selection-v1b-routes.integration.test.ts`, the `/advance` HTTP loop) got through N1→N7→N8 first-eval with a gate-valid BORDERLINE draft (`total_score:66`∈[60,72) → T1; `advance_to_package` floor is 60 so it stays gate-valid; dim spread<40 so only T1 fires) and the **first-pass loopback fired correctly** through the real harness (loopback + the new re-entry-guidance message). But the N7 feedback re-entry then **blocked with `N7_REQUIRED_SUPPORT_ARTIFACT_MISSING`** (harness-service.ts:3360-3367): the N7 gate-rejected readmission path requires a frozen **`n7_n8_debate_admission_review`** support (output contract `N8DebateAdmissionReviewSupport@v1`, deciding the debate level) in `semantic_artifacts` — a **`support_only`** artifact. The coordinator's `node_inputs.draft_payload` only records the **`model_draft_for_gate`** slot (N4/N6/N8 via `recordDraftSemanticArtifact`:884), so it cannot supply N7's debate-admission-review.
