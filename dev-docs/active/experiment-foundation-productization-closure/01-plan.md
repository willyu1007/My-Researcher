...
36. **Next authorization boundary — named-local only:** after explicit review, activate one dated max-40/no-cloud authorization, run normal T1-T4 and exact replay, require branch `18/9`, 40 new rows, protected tables unchanged and prohibited effects 0. Pin the returned server-owned Run manifest hash into the normal live scope, then run zero-cloud offline gates. Do not request STS or paid execution in the same authorization.
