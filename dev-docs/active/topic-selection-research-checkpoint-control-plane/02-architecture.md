...
- Direct EvidencePacket readiness and similarity-retrieval readiness are distinct contracts. Keyed locator resolution requires a current high-confidence quality assessment, usable key content, non-stale canonical source stages, and quote integrity; it does not require an embedding index because no similarity lookup occurs. Search-based discovery keeps the stricter active-index requirement. Conflating these boundaries would make inspectable direct evidence depend on an unrelated retrieval optimization and incorrectly block unindexed but reviewed canonical text.
- Role-specific retrieval and EvidencePacket resolution are separated by a deliberate materialization boundary. The request declares `local_snapshot_lexical` or `provider_hybrid`; the response and SearchRun log record that mode and the actual query-embedding provider request count. Local mode ranks only active indexed chunks belonging to the exact bound LiteratureSnapshot, supplies no query embedding, and therefore has zero provider calls. The Phase 8D shadow runner accepts only `local_snapshot_lexical` preparations with `provider_call_count=0`; a cached provider-hybrid path is still not the local validation lane. EvidenceMap selection is exact to TitleCard + SearchPlan + LiteratureSnapshot rather than assuming a TitleCard can have only one globally current map. Product retrieval may discover literature inside the snapshot but outside that exact reviewed EvidenceMap and records the divergence in SearchRun provenance, but the preparation result is `requires_evidence_materialization` until a current reviewed claim-bearing EvidenceUnit with a resolvable locator exists. Raw RAG hits, result summaries, and similarity scores never substitute for EvidenceMap review or keyed source resolution.
- A repair retry receives no client-authored free-form snapshot. `POST /topic-selection/research/arena/retry-snapshots` loads the predecessor session and snapshot, requires the current reviewed replacement EvidenceMap plus the unchanged canonical candidate refs, derives a support-only execution plan, and compiles a new snapshot whose sole typed delta ref is the replacement map. Ordinary session admission still enforces predecessor, changed hash, delta membership, and at-most-one retry; the compiler owns convenience, not new research authority.
- The HTTP arena lifecycle exposes a strict `POST /topic-selection/research/arena/sessions` open/replay boundary and `GET /topic-selection/research/arena/sessions/{arenaSessionId}` recovery boundary. Opening requires an existing InputSnapshot and checksum-valid execution-plan artifact bound to that snapshot, exact target equality, fixed participant roles, and support-only persistence; the stable session key replays only identical content. These routes add no checkpoint, accepted-risk, human-decision, promotion, or bridge writer.
- Independent first-pass execution uses a fan-out barrier, not the existing sequential debate-loop transcript. The opportunity scout and prior-art/topic killer share only the exact snapshot-bound candidate refs; each receives a different resolved EvidencePacket and no peer output. Both non-provider invocations must finish before any output artifact or `TopicSelectionResearchArenaRoleExecution` is admitted. Each output reviews every canonical candidate exactly once and cites only EvidenceUnits/literature present in that role's packet. The scout may emit provisional alternatives; they remain non-canonical and force `reframe_required`. The killer is prohibited from proposing, repairing, or rewriting candidates and must distinguish a coded evidence-backed drop from an evidence-insufficient park.
- Selection is comparative and singular at both role and synthesis boundaries. Each role may mark at most one canonical candidate `selected`; a candidate that merely survives criticism is parked unless the role's packet supports preferring it over visible alternatives. Deterministic synthesis returns `selected` only for exactly one candidate selected by both independent roles with no role-position dissent. Other candidates may remain consensus-parked with reopening conditions, preserving one active path plus delta-gated parked alternatives; they need not be falsely dropped to let the active path continue.
- Synthesis is deterministic and advisory. A candidate is `selected` only when both independent roles select it, `dropped` only when both issue coded drops, and otherwise `parked`; any selected/drop or selected/park conflict yields `evidence_expansion_required`. A scout proposal prevents selection and yields `reframe_required`. If every canonical candidate is jointly dropped, `none_viable` is a successful result. One snapshot-bound transcript preserves both execution refs/hashes, exposure hashes, findings, dissent, candidate dispositions, reopening conditions, and next delta. `support_only=true` remains invariant, and the runner has no dependency on checkpoint, HumanConfirmedDecision, AcceptedRisk, promotion, or bridge writers.
