...
- **做完整运行时**（非半成品）——要让"候选弱→升级 debate→正常继续"真正可走,必须 a–i 全做:
  - 原语:生成型任务,倾向 divergent_loop（explorer 探不同 framing → critic 挑战 → arbiter 综合子集）;亦可 bounded_sequence（复用 v1c N2 / v1b N8 范式）。**3.1 设计定（2026-06-19，用户两轮确认 + D-T127-02 锁定）：divergent_loop（3 角色 explorer→critic→arbiter）**——更贴合 N6 生成型探索任务;模板镜像 V1A divergent_loop debate 的角色编排与 scenario 形态,非 N8 bounded_sequence。**调和 DMP-10（单 core）**：共享 `bounded-debate-core` 现仅 bounded_sequence 固定序列走（装不下 divergent_loop;V1A 用独立 service）——本期**加法扩展** core（Option A，用户确认）：在同一 core 类新增 `runDivergentLoop` 方法（多 explorer 扇出→critic→arbiter issue-frame→final-synthesis 选子集），每 turn 复用现有 `generateRoleArtifact`;**`runLoop` + `generateRoleArtifact` 零 diff → N8/v1c-N2 byte-identity 构造性成立**;新增 `DivergentDebateStrategy` 扩展 bounded 的 per-turn hook 子集。N8 + v1c-N2 replay 守卫为 core-gen 前置硬门;**不复制 V1A 独立 service**（DMP-10 单 core、不建第二套）。
