...
- Reviewed current M7 source/schema/composition population. The existing RunRecipe is `materialize-only`, TrainingTaskSpec output keys are simulation-only, shared execution contracts admit only fake provenance, Prisma CHECKs reject `real_provider`, the worker validates only deterministic fake responses, and product composition always constructs the fake transport. Consequently the existing immutable Run cannot be used for real execution.
