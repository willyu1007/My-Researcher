...
- 2026-05-05: Added OpenAI extraction settings that reuse the redacted provider secret, with `default` = `gpt-5.4-mini` and `high_accuracy` = `gpt-5.5`; legacy `gpt-5-mini` / `gpt-5.2` settings are migrated on read.
