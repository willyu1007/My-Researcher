...
4. Implement the bounded default-off compatibility increment before any live job: extend the shared payload schema, materializer, official-SDK wire map, redacted manifest/hash and negative tests for exact `DataSources`, `Envs` and `CredentialConfig` bindings. Resolve the actual official `ImageUri` and an acceptable immutable provider image identity; the console display label alone is not sufficient.
5. Complete D preparation: create digest/size manifests for the workload archive and SciFact slice, upload them to content-addressed `input/` object names only in a separately confirmed window, and verify the final object refs without enabling `CreateJob`.
