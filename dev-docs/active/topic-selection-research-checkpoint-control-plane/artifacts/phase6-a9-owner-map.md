...
| Current-evidence primary NeedCandidate A | `need_candidate_ccdb06f3-aebc-49a3-8371-b3400204fd9e` / `t147-a9-pos-signed-depthwise-current-gap-v1`; open-loop signed `0→1` and `1→5` utility prediction; current, non-speculative, semantic group `8ea4d0270e8c1c587b80cea9b844993fa1d7fa9a222e397773d60178eb2cea10` |
| Current-evidence viable alternative C | `need_candidate_132054e7-48a7-416f-809d-dd71b32ed3bc` / `t147-a9-pos-feedback-stopping-current-gap-v1`; post-`k=1` feedback-conditioned stop-or-continue; current, non-speculative, semantic group `34d24cd975b3b11b8da1eb395d0ee156dc79201bbd90610cdb7c820ead7ad193` |
...
| Current gap policy / predecessor | `eligible_for_human_review`; two machine-viable candidates in two semantic groups; zero issues, required actions, or objections; actions `advance` / `hold` / `loopback` / `reject`; source refs include refreshed evidence checkpoint `research_checkpoint_19a6227d-3849-4d0e-ab6f-9e89e072b457` |
...
| Gap rebuild boundary proof | A is `ready_for_validation` / `needs_human_review`; C is `hypothesis` / `machine_checked`; both have null adjudication and ValidatedNeed refs. No new HumanConfirmedDecision, HumanPromotionDecision, PromotionDecision, bridge, PaperProject, provider call, or corpus mutation occurred. |
| Current v1c frontier | Machine Gate remains independently `ready_for_human_decision`, but the old promotion checkpoint is stale relative to the repaired gap lineage and must not be retried. The earliest unsatisfied point is now the exact strict-human decision on current gap checkpoint `research_checkpoint_de57273c-4b2e-4820-9f98-898920a6fe63`, followed by a fresh question/package/v1c lineage and newly snapshot-bound promotion confirmation. |

## Resume guard

The historical evidence decisions remain durable, both earlier candidate arenas remain readable, and their human-confirmed ValidatedNeed is preserved as provenance, but the old gap authority is upstream-stale relative to refreshed evidence checkpoint `research_checkpoint_19a6227d-3849-4d0e-ab6f-9e89e072b457`. Preserve the negative arena as a non-advancing control. The bounded two-paper acquisition, parsing, curation, and EvidenceMap refresh are complete, and downloader `max_byte_size` is restored to `1024`. Current-evidence candidates A and C and gap checkpoint `research_checkpoint_de57273c-4b2e-4820-9f98-898920a6fe63` are now materialized; do not create more candidates or mutate candidate facts before the exact packet is reviewed, because doing so would change the frozen arena hash. Resume by presenting snapshot `fceeff1592b8ebbdb9ceb3e5fac73615bfdf9c798924e8a763b53675bc34c748` and support packet `validation_packet_0ae21385-679b-4a5c-ab95-d24ed8d5aff0` for strict-human gap confirmation. Do not retry old promotion checkpoint `research_checkpoint_35406533-fac3-4cdb-b6d9-29296ca66c63`; after a successful gap decision, rebuild every downstream checkpoint so each carries the current predecessor ref. Do not invoke a provider, record a gap or promotion Human authority, or create a bridge without the corresponding new authorization boundary. The configured raw-file root is transient; if its OA PDFs disappear, obtain exact authorization and reacquire only those two assets through a new bounded dry-run rather than overwriting storage settings.
