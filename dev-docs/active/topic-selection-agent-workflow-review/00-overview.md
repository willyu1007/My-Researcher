...
- Progress: **活跃 backlog（承 T-128 W-08 移交 + T-088 对账移交，2026-07-05 首次落本包账面）**：
  - [x] ① 矩阵语义列（`executor_kind`/`default_execution_mode`/`debate_primitive`/`human_*`）接入一致性脚本（**DONE 2026-07-05**；**尾巴**：v1a 与 resource_sampling 行的语义列仍无结构化契约导出源、未校验——若后续补 v1a 导出需动 v1a harness contracts，另行切片）；
