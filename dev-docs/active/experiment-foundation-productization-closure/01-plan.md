...
10. [ ] Complete sequence-8 paid verification. The console-default comparison has isolated and implemented the final output mount's omitted `MountAccess` differential; named-local sequence 8, exact replay and read-only `GetImage` have passed. Obtain fresh bounded STS, submit the exact two Jobs once, require terminal success for both, collect both exact outputs and stop for provider escalation with RequestIds/wire evidence if the same rejection recurs rather than inventing sequence 9/10.
11. [ ] Complete the personal-use landing through backend/API/CLI: prove exact Run/Attempt/result visibility, zero-new replay, deterministic failure/reconcile/cancel coverage, credential/Job/database/cost cleanup and an operating/recovery runbook. Desktop UI is not required.
...
15. [ ] Resume the owner-authorized sequence-8 paid verification on the original workstation. Cloud Shell bootstrap permission and one functional replacement-session check are complete. The current workstation cannot read the authoritative sequence-8 database, and no STS or paid runner was used there. Pull `main`, verify the reviewed named-local target and exact zero-cloud `offline-preflight`, then require another fresh STS, retain the active whitelist-only error/wire observers, invoke the runner once and stop at the call ceiling. If execution moves beyond 2026-07-30, obtain a new dated authorization first.
