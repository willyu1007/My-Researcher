...
| Gap rebuild boundary proof | A is closed / `human_confirmed`, C remains `hypothesis` / `machine_checked` as the viable alternative, the adjudication validated A, and HumanConfirmNeed created the current ValidatedNeed. No new v1b input bundle, downstream question/package/v1c owner, HumanPromotionDecision, PromotionDecision, bridge, PaperProject, provider call, or corpus mutation occurred. |
| Current v1c frontier | Machine Gate remains independently `ready_for_human_decision`, but the old question/package/v1c/promotion chain is stale relative to the repaired gap lineage and must not be retried. The earliest unsatisfied point is authorization to publish a new v1b input bundle from the current ValidatedNeed and rebuild a fresh question lineage; after strict-human question advancement, rebuild package/v1c and present a newly snapshot-bound promotion confirmation. |

## Resume guard

The historical evidence decisions remain durable, both earlier candidate arenas remain readable, and their human-confirmed ValidatedNeed is preserved as provenance, but the old gap authority is upstream-stale relative to refreshed evidence checkpoint `research_checkpoint_19a6227d-3849-4d0e-ab6f-9e89e072b457`. Preserve the negative arena as a non-advancing control. The bounded two-paper acquisition, parsing, curation, and EvidenceMap refresh are complete, and downloader `max_byte_size` is restored to `1024`. Current-evidence gap checkpoint `research_checkpoint_de57273c-4b2e-4820-9f98-898920a6fe63` is decided `advance`; A is now the human-confirmed source of ValidatedNeed `validated_need_a58766a3-d482-4dc3-9b0b-cdeac9e3d095`, while C remains the unselected viable alternative. Do not retry old promotion checkpoint `research_checkpoint_35406533-fac3-4cdb-b6d9-29296ca66c63` or its stale downstream lineage. Resume only with new authorization to publish a v1b input bundle from the new ValidatedNeed and rebuild the v1b question chain, stopping at every strict-human authority; after a current question advances, rebuild package/v1c before presenting promotion again. Do not invoke a provider or create a bridge without the corresponding new authorization boundary. The configured raw-file root is transient; if its OA PDFs disappear, obtain exact authorization and reacquire only those two assets through a new bounded dry-run rather than overwriting storage settings.
